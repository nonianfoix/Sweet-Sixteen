   ...
   case 'ADVANCE_TOURNAMENT_ROUND': {
        ...
        if (wasChampionshipSimulated) {
            ...
            const recapData = {
                ...
                prestige: userTeamHistory.prestige,
                ...
                operationalExpenses: finalRevenue.operationalExpenses,
                netIncome: finalRevenue.netIncome,
                cashOnHand: finalRevenue.cashOnHand,
                revenueBreakdown: finalRevenue,
                attendance,
                sponsor,
                tournamentChampion: newTournament?.champion ?? null,
                tournamentRunnerUp,
            };
...
const SeasonRecapModalV2 = ({ recapData, onClose }: { recapData: GameState['seasonRecapData'], onClose: () => void }) => {
    ...
    const prestigeStyle = prestigeChange === 0 ? {} : { color: prestigeChange > 0 ? 'green' : 'red', fontWeight: 'bold' as const };
    const prestigeText = prestigeChange > 0 ? `▲ +${prestigeChange}` : `▼ ${prestigeChange}`;
    const reputationStyle = reputationChangeValue === 0 ? {} : { color: reputationChangeValue > 0 ? 'green' : 'red', fontWeight: 'bold' as const };
    const reputationText = reputationChangeValue >= 0 ? `+${reputationChangeValue}` : `${reputationChangeValue}`;
...
export default function App() {
  ...
  const renderContent = () => {
    ...
    case GameStatus.GAME_LOG:
      ...
      case GameStatus.SEASON_RECAP: return <SeasonRecapModalV2 recapData={state.seasonRecapData} onClose={() => dispatch({type: 'EVALUATE_OFFSEASON'})}/>;
