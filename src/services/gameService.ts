...
import {
    ...
    SponsorRevenue, Prices, Finances, ProgramWealth, NilNegotiationCandidate, AlumniProfile,
    ...
    PhysicalProfile, TranslatableSkills, NBAReadiness, ProLearningCurve, CollegePerformance, DraftScoutReport,
    // Season Recap Types
    SeasonRecapData, PlayerDevelopmentDNA, OfferPitchType, RelationshipType, TimelineEvent, CoachPerformanceIndex
} from '../types';
...
import { normalizePlayerName, getProgramWealth } from './utils';
...
const adjustRookieForNBA = (player: Player): Player => {
    const collegeOVR = typeof player.overall === 'number' ? player.overall : calculateOverall(player.stats);
    const age = player.age || 19;
    
    // Derive college experience from year if not set
    const collegeYears: 1 | 2 | 3 | 4 | 5 = player.collegeExperience || (
        player.year === 'Sr' ? 4 : 
        player.year === 'Jr' ? 3 : 
        player.year === 'So' ? 2 : 1
    );
    
    const physicalProfile = player.physicalProfile || 'Average';
    const athleticismTier = player.athleticismTier || 'B';
    const position = player.position;
    
    // Tier-based drop (higher OVR = more polished = smaller drop)
    const baseDropByTier = (() => {
        if (collegeOVR >= 92) return randomBetween(5, 12);   // Elite prospects
        if (collegeOVR >= 85) return randomBetween(8, 16);   // Lottery talent
        if (collegeOVR >= 78) return randomBetween(12, 20);  // First round
        return randomBetween(15, 25);                         // Second round/undrafted
    })();
    
    // Experience modifier (4-year players are more NBA-ready)
    const experienceBonus = Math.max(0, (collegeYears - 1) * -2); // -2 per year beyond Fr
    
    // Position modifier (bigs take longer to adjust, guards adapt faster)
    const positionMod = ['C', 'PF'].includes(position) ? 3 : 
                        ['SF'].includes(position) ? 1 : 0;
    
    // Physical profile affects transition (elite frames adapt easier)
    const physicalMod = physicalProfile === 'Elite' ? -2 :
                        physicalProfile === 'Prototypical' ? -1 :
                        physicalProfile === 'Undersized' ? 3 : 0;
    
    // Athleticism affects adjustment speed (elite athletes adapt faster)
    const athleteMod = athleticismTier === 'A' ? -2 :
                       athleticismTier === 'D' ? 2 : 0;
    
    const totalDrop = clamp(baseDropByTier + experienceBonus + positionMod + physicalMod + athleteMod, 5, 28);
    
    // Potential boost based on age (younger = more upside)
    const potentialBoost = age <= 19 ? randomBetween(12, 22)
                         : age <= 21 ? randomBetween(8, 16)
                         : randomBetween(3, 10);
    
    // Calculate NBA readiness profile
    const nbaReadiness: NBAReadiness = player.nbaReadiness || {
        physicalReadiness: physicalProfile === 'Elite' ? 85 : 
                          physicalProfile === 'Prototypical' ? 75 : 
                          physicalProfile === 'Average' ? 65 : 55,
        mentalReadiness: clamp(40 + collegeYears * 12 + (player.coachability || 0) * 0.2, 40, 95),
        skillReadiness: clamp(collegeOVR - 15 + (collegeYears * 3), 50, 95),
        projectedAdjustmentCurve: collegeYears >= 3 ? 'Fast' : collegeYears === 2 ? 'Average' : 'Slow',
    };
    
    // Initialize pro learning curve
    const proLearningCurve: ProLearningCurve = {
        seasonsInNBA: 0,
        adaptationProgress: 0,
        hasBreakoutSeason: false,
        peakReached: false,
    };
    
    // Apply the drop preferring weakness stats when possible
    const targetOverall = clamp(collegeOVR - totalDrop, 55, 90);
    const adjusted = { ...player, stats: { ...player.stats } };
    
    // Apply stat drops intelligently based on translatableSkills if available
    if (player.translatableSkills) {
        // Drop weakness stat more aggressively
        const weaknessStat = player.translatableSkills.weakness;
        adjusted.stats[weaknessStat] = clamp(adjusted.stats[weaknessStat] - randomBetween(8, 15), 40, 99);
    }
    
    // Tune overall to target
    ...
    adjusted.potential = clamp(Math.max(adjusted.overall + 5, (player.potential ?? collegeOVR) + potentialBoost), adjusted.overall, 99);
    
    // Set NBA-specific fields
    adjusted.nbaReadiness = nbaReadiness;
    adjusted.proLearningCurve = proLearningCurve;
    adjusted.collegeExperience = collegeYears;
...
export const createNilCollectiveProfile = (team: Team): NilCollectiveProfile => {
    ...
    const wealth = resolveWealth(team);
    const donorEnergy = wealth.donorMomentum ?? 10;
    ...
    const alumniContribution = Math.round(base * 0.25 + wealth.boosterPool * 15000);
    return {
...
const resolveWealth = (team: Team): ProgramWealth =>
    getProgramWealth(
        team,
        () => seedProgramWealth(team.name, team.prestige, team.conference, team.fanInterest)
    );

export const recalculateProgramWealth = (team: Team, tournament: Tournament | null): ProgramWealth => {
    const previous = resolveWealth(team);
...
const computeAlumniDonationPass = (team: Team): { total: number; nil: number; facilities: number; endowment: number; reasons: string[] } => {
    ...
    const wealth = resolveWealth(team);
    const boosterSentiment = clamp(wealth.boosterSentiment ?? 50, 0, 100);
    ...
    const baseline = Math.round(((wealth.donationLevel ?? 50) * 12000 + wealth.boosterPool * 65000) * sentimentMultiplier);
...
const applyOffseasonAlumniBoosterPass = (team: Team, season: number): Team => {
    ...
    const normalizedWealth = resolveWealth(withWarChest);
    ...
    const baseNetwork = normalizedWealth.alumniNetwork ?? {
        strength: clamp(15 + (normalizedWealth.donationLevel - 50) * 0.5, 0, 100),
        lastDonation: 0,
...
export const getWealthRecruitingBonus = (team: Team | null): number => {
    if (!team) return 0;
    const wealth = resolveWealth(team);
    const endowmentBonus = Math.floor(wealth.endowmentScore / 18);
    const donationBonus = Math.floor(wealth.donationLevel / 20);
    const boosterBonus = Math.round(wealth.boosterPool * 0.8);
...
export const getWealthTrainingBonus = (team: Team | null): number => {
    if (!team) return 0;
    const wealth = resolveWealth(team);
    const endowmentBonus = Math.floor(wealth.endowmentScore / 20);
    const donationBonus = Math.floor(wealth.donationLevel / 24);
    const boosterBonus = Math.round(wealth.boosterPool * 0.5);
    return endowmentBonus + donationBonus + boosterBonus;
};
...
export const createRecruit = (): Recruit => {
    ...
    // Calculate physical profile based on height/wingspan for position
    const positionHeightNorms: Record<string, { avg: number; elite: number }> = {
        'PG': { avg: 74, elite: 77 },
        'SG': { avg: 77, elite: 80 },
        'SF': { avg: 79, elite: 82 },
        'PF': { avg: 81, elite: 84 },
    ...
    const posNorm = positionHeightNorms[playerPart.position] || { avg: 78, elite: 81 };
    const heightDiff = height - posNorm.avg;
    const wingspanBonus = wingspan - height; // Positive = long arms
    const physicalScore = heightDiff + (wingspanBonus * 0.5) + (athleticismTier === 'A' ? 2 : athleticismTier === 'D' ? -2 : 0);
    const physicalProfile: PhysicalProfile = 
        physicalScore >= 4 ? 'Elite' :
        physicalScore >= 1 ? 'Prototypical' :
        physicalScore >= -2 ? 'Average' :
        'Undersized';

    // Calculate translatable skills based on current stats
    const statEntries = Object.entries(playerPart.stats) as [keyof Player['stats'], number][];
    const sortedStats = [...statEntries].sort((a, b) => b[1] - a[1]);
    const weakestStats = [...statEntries].sort((a, b) => a[1] - b[1]);
    const translatableSkills: TranslatableSkills = {
        primary: sortedStats[0][0],
        secondary: sortedStats[1][0],
        weakness: weakestStats[0][0],
    };
    ...
    return {
        ...
        archetype,
        interestMap: {},  // Initialize empty, populated during recruiting
...
/**
 * Calculate team rating from actual roster. Used when nbaTeams exist in state.
 * Returns a weighted average of the top 8 players' overalls plus depth/experience bonuses.
 */
export const calculateNBATeamRating = (team: Team): number => {
    if (!team.roster?.length) return 72; // Default rating if no roster
    
    // Sort players by overall and take top 8
    const sorted = [...team.roster].sort((a, b) => (b.overall || 70) - (a.overall || 70));
    const core = sorted.slice(0, 8);
    
    // Minutes-weighted average (starters count more)
    const weights = [0.22, 0.18, 0.15, 0.13, 0.12, 0.08, 0.07, 0.05];
    const weightedSum = core.reduce((sum, player, index) => {
        const weight = weights[index] || 0.05;
        return sum + (player.overall || 70) * weight;
    }, 0);
    
    // Depth bonus: Having 12+ players helps
    const depthBonus = team.roster.length >= 12 ? 2 : team.roster.length >= 10 ? 1 : 0;
    
    // Experience bonus: Veterans help
    const experiencedPlayers = core.filter(p => (p.experience || 0) >= 3).length;
    const experienceBonus = Math.min(5, experiencedPlayers);
    
    // Chemistry/prestige adjustment (optional)
    const prestigeBonus = team.prestige ? Math.max(0, (team.prestige - 70) * 0.05) : 0;
    
    return clamp(Math.round(weightedSum + depthBonus + experienceBonus + prestigeBonus), 60, 99);
};

/**
 * Simulate an NBA season. If nbaTeams are provided with actual rosters,
 * team ratings will be calculated from real player data instead of random.
 */
export const simulateNBASeason = (season: number, nbaTeams?: Team[]): NBASimulationResult => {
    // Build team simulations, using real roster data if available
    const teams: NBATeamSimulation[] = NBA_TEAMS.map(info => {
        const existingTeam = nbaTeams?.find(t => t.name === info.name);
        const rating = existingTeam 
            ? calculateNBATeamRating(existingTeam)
            : clamp(72 + randomBetween(-8, 12), 65, 98); // Fallback to random
...
const calculateFanArchetypeDemand = (
    ...
    const homeWealth = resolveWealth(homeTeam);
    ...
    const boosterSignal =
        clamp(((homeWealth.donationLevel ?? 45) - 35) / 70, -0.2, 0.9) +
        clamp((homeWealth.boosterPool ?? 6) / 60, 0, 0.4) +
...
export const calculateTeamRevenue = (team: Team, tournament: Tournament | null, lastSeasonRecord?: UserSeasonRecord): Finances => {
    ...
    const wealth = resolveWealth(team);
    const donationRevenue = (wealth.donationLevel * 15000) + (wealth.boosterPool * 40000);
    const endowmentSupport = wealth.endowmentScore * 20000;
    ...
    if (team.budget?.allocations) {
        ...
        travelExpenses = Math.round(baseOperationalBucket * 0.18 * travelWeight);
        recruitingExpenses = Math.round(baseOperationalBucket * 0.20 + wealth.boosterPool * 15000);
...
export const initializeGameWorld = (userTeamName: string) => {
  ...
  let allTeams: Team[] = SCHOOLS.map(name => {
    ...
    const wealth: ProgramWealth = institutional
        ...
        : wealthBase;
    const headCoach = createHeadCoach(prestige);
    const alumniRegistry = generateBaselineAlumniRegistry({ name, prestige, conference } as Team);
        ...
        record: { wins: 0, losses: 0 },
        seasonStats: { wins: 0, losses: 0, confWins: 0, confLosses: 0 },
  ...
  generatedSchedule.regularSeasonDates.forEach((dateISO, idx) => {
      ...
      const anchorProgramByState = new Map<string, string>();
      allTeams.forEach(team => {
          const state = normalizeStateCode(team.state || '');
          if (!state) return;
          const current = anchorProgramByState.get(state);
          if (!current) {
              anchorProgramByState.set(state, team.name);
              return;
          }
          const currentTeam = allTeams.find(t => t.name === current);
          const currentPrestige = currentTeam?.prestige ?? 0;
          if ((team.prestige ?? 0) > currentPrestige) {
              anchorProgramByState.set(state, team.name);
          }
      });
      recruits = recruits.map(r => {
          const state = normalizeStateCode(r.hometownState || r.homeState || r.state || '');
          const anchorProgram = anchorProgramByState.get(state);
          return anchorProgram ? { ...r, hometownAnchorProgram: anchorProgram } : r;
      });
...
const getTeamAcademicScore_Duplicate = (team: Team): number => {
    const wealth = resolveWealth(team);
    const endowment = wealth.endowmentScore ?? 50;
    const supportBoost = (team.finances?.endowmentSupport ?? 0) / 5000;
    return clamp(endowment + supportBoost, 0, 100);
};

const getTeamDevelopmentScore_Duplicate = (team: Team): number => {
...
const getTeamMarketScore_Duplicate = (team: Team): number => {
    const base = team.prestige ?? 50;
    const donorBonus = resolveWealth(team).donorMomentum ?? 0;
    const capacityBoost = (team.facilities?.arena?.capacity ?? 10000) / 160;
    return clamp(base + donorBonus + capacityBoost, 0, 100);
};

const getTeamCommunityScore_Duplicate = (team: Team): number => {
...
export const calculateRecruitInterestScore = (recruit: Recruit, team: Team, context: { gameInSeason: number, isSigningPeriod?: boolean }, userCoachSkills?: string[]): number => {
	    ...
	    let prestigeFactor = (prestige - 50) * (0.25 + exposureWeight * 0.45);
        const marketScore = getTeamMarketScore(team);
        const prestigeNormalized = clamp(prestige / 100, 0, 1);
        ...
        const fameSeekerMultiplier = recruit.archetype === 'FameSeeker' ? 2 : 1;
        const awarenessFactorAdjusted = awarenessFactor * fameSeekerMultiplier;
        const exposureBonusAdjusted = exposureBonus * fameSeekerMultiplier;
        const expectedNilBonusAdjusted = expectedNilBonus; // FameSeeker cares about exposure/brand, not necessarily NIL.
    ...
    const inSeasonBonus = context.gameInSeason <= 31 ? context.gameInSeason * 0.1 : 0;
	    ...
	    const alumniNilBonus = (resolveWealth(team).alumniNetwork?.strength ?? 0) * 0.08 * nilWeight;
        ...
        case 'Mercenary':
            {
                ...
                const threshold = 250000 + recruit.stars * 200000;
                const shortfall = Math.max(0, threshold - nilBudget);
                archetypeBonus += wealthBonus * 1.5;
                if (shortfall > 0) {
                    archetypeBonus -= clamp(shortfall / 35000, 0, 55);
                }
            }
            break;
        case 'FameSeeker':
            archetypeBonus += (marketScore - 50) * 0.2;
            break;
        case 'HometownHero':
            ...
            if (isHomeState) {
                const isAnchor = recruit.hometownAnchorProgram && recruit.hometownAnchorProgram === team.name;
                archetypeBonus += (38 + proximityWeight * 42) * preferenceMultiplier + (isAnchor ? 18 : 0);
            } else {
                ...
                const distanceMultiplier =
                    ...
                    1.35;
                const basePenalty = (26 + proximityWeight * 34) * preferenceMultiplier * distanceMultiplier;
    ...
    if (recruit.archetype === 'Mercenary') {
        const nilBudget = team.nilBudget ?? 0;
        const threshold = 250000 + recruit.stars * 200000;
        if (nilBudget < threshold) {
            dealbreakerPenalty = Math.max(dealbreakerPenalty, 35);
        }
    }
	    ...
	    let rawScore =
	        baseInterest +
	        awarenessFactorAdjusted +
	        (prestigeFactor * fameSeekerMultiplier) +
            exposureBonusAdjusted +
            expectedNilBonusAdjusted +
	        recordFactor +
	        positionNeedFactor +
	        -playingTimePenalty +
	        inSeasonBonus +
...
export const processRecruitingWeek = (
    ...
    const signingEnabled = isSigningPeriod && week >= 8;
    const decisionPressure = getDecisionPressure(week, signingEnabled);

    const teamCommitCounts = new Map<string, number>();
    updatedRecruits.forEach((r: Recruit) => {
        r.isSigned = Boolean(r.isSigned || r.recruitmentStage === 'Signed' || r.signWeek != null);
        r.verbalLevel = r.verbalLevel || (r.verbalCommitment ? (r.softCommitment ? 'SOFT' : 'HARD') : 'NONE');
        r.lockStrength = Number.isFinite(r.lockStrength) ? r.lockStrength : 0;
        r.activeOfferCount = Number.isFinite(r.activeOfferCount)
            ? r.activeOfferCount
            : ((r.cpuOffers?.length || 0) + (r.userHasOffered ? 1 : 0));
        if (r.verbalCommitment && r.verbalDay == null) {
            r.verbalDay = r.commitWeek ?? week;
        }
    ...
    const sortedRecruits = [...updatedRecruits].sort((a, b) => (b.overall + b.potential) - (a.overall + a.potential));
        ...
        if (!options?.skipCpuActions) teams.forEach(team => {
	        ...
	        const boardSize = isDukeTier
                ? (early ? 45 : mid ? 32 : 24)
                : (early ? 90 : mid ? 70 : 55);
            const maxActiveOffers = isDukeTier ? 18 : prestige >= 80 ? 25 : 40;
            let activeOffers = updatedRecruits.reduce((count: number, r: Recruit) => {
                if ((r.cpuOffers || []).includes(team.name) && !r.isSigned) return count + 1;
                return count;
            }, 0);

            const canPursueRecruit = (r: Recruit): boolean => {
                if (r.isSigned) return false;
                if ((r.declinedOffers || []).includes(team.name)) return false;
                if (isDukeTier) {
                    const recruitAcademicPreference = r.preferredProgramAttributes?.academics ?? 50;
                    if (recruitAcademicPreference < 40 || r.personalityTrait === 'Family Feud') return false;
                }
                if (!r.verbalCommitment) return true;
                if (r.verbalCommitment === team.name) return false;

                const committedTeam = teamsByName.get(r.verbalCommitment);
                const committedPrestige = committedTeam?.prestige ?? 50;
                const committedNil = committedTeam?.nilBudget ?? 0;
                const prestigeDelta = prestige - committedPrestige;
                const nilDelta = (team.nilBudget ?? 0) - committedNil;

                const significantUpgrade = prestigeDelta >= 6 || nilDelta >= 400000;
                if (!significantUpgrade) return false;
                if (r.archetype === 'Mercenary' && nilDelta < 250000) return false;
                return true;
            };

            const rescindLowestMomentumOffer = (): boolean => {
                if (activeOffers < maxActiveOffers) return true;
                const candidates = updatedRecruits
                    .filter((r: Recruit) => (r.cpuOffers || []).includes(team.name))
                    .filter((r: Recruit) => !r.isSigned)
                    .filter((r: Recruit) => !r.verbalCommitment || r.verbalCommitment !== team.name)
                    .filter((r: Recruit) => {
                        const hasActivePackagePartnerOffer = (r.relationships || []).some(link => {
                            ...
                            if (other.isSigned) return false;
                            return (other.cpuOffers || []).includes(team.name);
                        });
                        return !hasActivePackagePartnerOffer;
                    })
                    .map((r: Recruit) => {
                        const momentum = r.teamMomentum?.[team.name] ?? 0;
                        const score = calculateRecruitInterestScore(r, team, { gameInSeason: week, isSigningPeriod: signingEnabled }, userCoachSkills);
                        return { r, momentum, score };
                    })
                    .sort((a, b) => (a.momentum - b.momentum) || (a.score - b.score));

                const loser = candidates[0]?.r;
                if (!loser) return false;
                loser.cpuOffers = (loser.cpuOffers || []).filter(x => x !== team.name);
                if (!loser.offerHistory) loser.offerHistory = [];
                for (let i = loser.offerHistory.length - 1; i >= 0; i--) {
                    const entry = loser.offerHistory[i]!;
                    if (entry.teamName === team.name && !entry.revoked) {
                        loser.offerHistory[i] = { ...entry, revoked: true };
                        break;
                    }
                }
	        ...
	        for (const r of board) {
	            ...
	            if (!canPursueRecruit(r)) continue;
	        ...
	        for (const target of offerCandidates) {
                ...
                if (!rescindLowestMomentumOffer()) break;
    ...
    if (userTeam) {
        const userPrestige = userTeam.recruitingPrestige ?? userTeam.prestige ?? 50;
        const userMaxActiveOffers = userPrestige >= 94 ? 18 : userPrestige >= 80 ? 25 : 40;
        const pruneUserOfferIfNeeded = (): boolean => {
            const active = updatedRecruits.filter((r: Recruit) => r.userHasOffered && !r.isSigned).length;
            if (active < userMaxActiveOffers) return true;
            const candidates = updatedRecruits
                .filter((r: Recruit) => r.userHasOffered && !r.isSigned)
                .filter((r: Recruit) => !r.verbalCommitment || r.verbalCommitment !== userTeamName)
                .map((r: Recruit) => ({
                    r,
                    momentum: r.teamMomentum?.[userTeamName] ?? 0,
                    interest: r.interest ?? 0,
                }))
                .sort((a, b) => (a.momentum - b.momentum) || (a.interest - b.interest));
            const loser = candidates[0]?.r;
            if (!loser) return false;
            loser.userHasOffered = false;
            if (!loser.offerHistory) loser.offerHistory = [];
            for (let i = loser.offerHistory.length - 1; i >= 0; i--) {
                const entry = loser.offerHistory[i]!;
                if (entry.teamName === userTeamName && !entry.revoked) {
                    loser.offerHistory[i] = { ...entry, revoked: true };
                    break;
                }
            }
        ...
        for (const r of updatedRecruits) {
            if (
                ...
                if (currentContactsMade + offerCost <= actualContactPoints) {
                        if (!pruneUserOfferIfNeeded()) break;
                        ...
                        r.activeOfferCount = (r.cpuOffers?.length || 0) + 1;
	                }
            }
        }
    }
            ...
            .map(teamName => {
                ...
                return {
                    ...
                    score: calculateRecruitInterestScore(
                        r,
                        team,
                        { gameInSeason: week, isSigningPeriod: signingEnabled },
    ...
    updatedRecruits.forEach((r: Recruit) => {
        ...
        r.relationships.forEach(link => {
            ...
            const jointCommitThreshold = (135 - decisionPressure * 12) - (signingEnabled ? 8 : 0);
            const individualMinAcceptable = clamp(58 - decisionPressure * 6 - (signingEnabled ? 4 : 0), 45, 65);
            ...
            if (
                ...
                r.verbalDay = week;
                other.verbalDay = week;
                r.lockStrength = clamp(r.lockStrength ?? 0.12, 0, 1);
                other.lockStrength = clamp(other.lockStrength ?? 0.12, 0, 1);
                ...
                r.softCommitment = !signingEnabled && week <= 8;
                other.softCommitment = r.softCommitment;
                r.verbalLevel = r.softCommitment ? 'SOFT' : 'HARD';
                other.verbalLevel = other.softCommitment ? 'SOFT' : 'HARD';
                ...
                if (signingEnabled) {
                    ...
                    other.isSigned = true;
                    r.lockStrength = 1;
                    other.lockStrength = 1;
                    r.verbalLevel = 'HARD';
                    other.verbalLevel = 'HARD';
                    ...
                    logTelemetryEvent('SigningEvent', { recruitId: r.id, recruitName: r.name, school: bestJointSchool.name, week });
                    logTelemetryEvent('SigningEvent', { recruitId: other.id, recruitName: other.name, school: bestJointSchool.name, week });
                    if (!r.recruitingEvents) r.recruitingEvents = [];
                    if (!other.recruitingEvents) other.recruitingEvents = [];
                    r.recruitingEvents.push({ type: 'Sign', week, to: bestJointSchool.name });
                    other.recruitingEvents.push({ type: 'Sign', week, to: bestJointSchool.name });
                } else {
        ...
        if (allOffers.length === 0) return;
            ...
            // Recruitment inertia: verbal recruits gradually "lock in" (poachable until signed).
            if (wasCommitted && committedSchool && !r.isSigned) {
                r.verbalLevel = r.softCommitment ? 'SOFT' : 'HARD';
                const relationshipWeight = clamp((r.motivations?.relationship ?? 50) / 100, 0, 1);
	        ...
	        if (wasCommitted && committedSchool && !signingEnabled) {
                ...
                if (shareGap > 15) {
                    const delta = shareGap - 15;
                    const sigmoid = 1 / (1 + Math.exp(-0.12 * delta));
                    const flipChance = clamp((1 - clamp(r.lockStrength ?? 0, 0, 1)) * sigmoid, 0, 0.75);
                    if (Math.random() < flipChance) {
                        const from = committedSchool;
                        const to = bestAlternative.name;
                        r.verbalCommitment = to;
                        r.verbalDay = week;
                        r.lockStrength = clamp(0.12 + (r.lockStrength ?? 0) * 0.25, 0, 1);
                        r.softCommitment = week <= 8;
                        r.verbalLevel = r.softCommitment ? 'SOFT' : 'HARD';
                        r.recruitmentStage = r.softCommitment ? 'SoftCommit' : 'HardCommit';
                        r.lastRecruitingNews = `${r.name} flipped from ${from} to ${to}.`;
                        r.isTargeted = false;
                        teamCommitCounts.set(from, Math.max(0, (teamCommitCounts.get(from) || 0) - 1));
                        teamCommitCounts.set(to, (teamCommitCounts.get(to) || 0) + 1);
                        if (to === userTeamName && r.userHasOffered) {
                            r.userHasOffered = false;
                        }
                        logTelemetryEvent('FlipEvent', {
                            recruitId: r.id,
                            recruitName: r.name,
                            from,
                            to,
                            ...
                            shareGap,
                            scoreGap,
                            lockStrength: r.lockStrength,
                        });
                        if (!r.recruitingEvents) r.recruitingEvents = [];
                        r.recruitingEvents.push({ type: 'Flip', week, from, to });
                        return;
                    }
                }
                ...
                const baseChance =
	                ...
	                if (Math.random() < chance) {
                        logTelemetryEvent('DecommitEvent', {
                            recruitId: r.id,
                            recruitName: r.name,
                            from: committedSchool,
                            week,
                            shareGap,
                            scoreGap,
                            lockStrength: r.lockStrength ?? 0,
                        });
                        if (!r.recruitingEvents) r.recruitingEvents = [];
            ...
            const signingCommitThreshold = 65 - (signingDay * 5);
	            if (topOffer.score > signingCommitThreshold) {
                    ...
                    r.verbalLevel = 'HARD';
                    r.lockStrength = 1;
                    r.verbalDay = r.verbalDay ?? week;
	                r.signWeek = week;
	                if (!r.commitWeek) r.commitWeek = week;
	                r.lastRecruitingNews = `${r.name} signed with ${topOffer.name}.`;
                    logTelemetryEvent('SigningEvent', { recruitId: r.id, recruitName: r.name, school: topOffer.name, week });
                    if (!r.recruitingEvents) r.recruitingEvents = [];
                    r.recruitingEvents.push({ type: 'Sign', week, to: topOffer.name });
	                if (r.userHasOffered) {
	                    r.userHasOffered = false;
	                }
	            }
	    ...
	    updatedRecruits.forEach((r: Recruit) => {
	        ...
	        if (signingEnabled && r.verbalCommitment) {
	            r.recruitmentStage = 'Signed';
                if (!r.isSigned) {
                    r.isSigned = true;
                    r.verbalLevel = 'HARD';
                    r.lockStrength = 1;
                    logTelemetryEvent('SigningEvent', { recruitId: r.id, recruitName: r.name, school: r.verbalCommitment, week });
                    if (!r.recruitingEvents) r.recruitingEvents = [];
                    r.recruitingEvents.push({ type: 'Sign', week, to: r.verbalCommitment || undefined });
                }
                r.signWeek = r.signWeek ?? week;
                return;
	        }
        ...
        updatedRecruits.forEach((r: Recruit) => {
            r.activeOfferCount = (r.cpuOffers?.length || 0) + (r.userHasOffered ? 1 : 0);
        });
...
export const runSimulationForDate = (
    ...
    updatedRecruits: Recruit[], 
    recruitingEvents: TimelineEvent[]
} => {
    ...
    const gameLogs: GameBoxScore[] = [];
    const isConferenceByGameId = new Map<string, boolean>();
    const newUserTeamAttendance: GameAttendanceRecord[] = [];
    const teamsCopy: Team[] = JSON.parse(JSON.stringify(teams));
    
    // Ensure seasonStats exists for all teams (migration/safeguard)
    teamsCopy.forEach(t => {
        if (!t.seasonStats) {
            t.seasonStats = { wins: t.record?.wins || 0, losses: t.record?.losses || 0, confWins: 0, confLosses: 0 };
        }
    });
    ...
    // --- PHASE 1: RECRUITING DAILY UPDATE ---
    const context: RecruitingContext = {
        gameInSeason: state.gameInSeason,
        currentDate: dateISO,
        isSigningPeriod: false, // TODO: Check season anchors
        recentGameResults: new Map(), // Populated after games if needed, or previous day
        newsEvents: [], // Filter from state.timeline if needed
        commitsTodayCount: 0, // Will track
        allTeams: teams // Pass all teams for scoring
    };
    
    const { updatedRecruits, events: recruitingEvents } = processRecruitingDay(state.recruits, context);
    // ----------------------------------------
    ...
    for (const event of eventsToday) {
        ...
        // Use deterministic Game ID to match App.tsx schedule view hash
        const gameId = `S${state.season}G${week}-${homeTeam.name}v${awayTeam.name}`;
        const isUserGame = homeTeam.name === state.userTeam?.name || awayTeam.name === state.userTeam?.name;
        const skillsToPass = isUserGame ? userCoachSkills : undefined;
        const simulatedGameResult = simulateGame(homeTeam, awayTeam, gameId, undefined, skillsToPass);
        gameLogs.push(simulatedGameResult);
        isConferenceByGameId.set(gameId, !!event.payload?.isConference);
    ...
    gameLogs.forEach(boxScore => {
        ...
        if (boxScore.homeScore > boxScore.awayScore) {
            ...
            // Update Team Season Stats for recap
            homeTeam.seasonStats.wins++;
            awayTeam.seasonStats.losses++;
            if (isConferenceByGameId.get(boxScore.gameId)) {
                homeTeam.seasonStats.confWins++;
                awayTeam.seasonStats.confLosses++;
            }
            ...
            if (isConferenceByGameId.get(boxScore.gameId)) {
                awayTeam.seasonStats.confWins++;
                homeTeam.seasonStats.confLosses++;
            }
        ...
        [...boxScore.homeTeamStats, ...boxScore.awayTeamStats].forEach(playerStat => {
            ...
            if (player) {
                ...
                player.seasonStats.assists += playerStat.stats.assists;
                player.seasonStats.minutes = (player.seasonStats.minutes || 0) + (playerStat.stats.minutes ?? 0);
                player.seasonStats.fieldGoalsMade += playerStat.stats.fieldGoalsMade;
                player.seasonStats.fieldGoalsAttempted += playerStat.stats.fieldGoalsAttempted;
                player.seasonStats.threePointersMade += playerStat.stats.threePointersMade;
                player.seasonStats.threePointersAttempted += playerStat.stats.threePointersAttempted;
                player.seasonStats.freeThrowsMade += playerStat.stats.freeThrowsMade;
                player.seasonStats.freeThrowsAttempted += playerStat.stats.freeThrowsAttempted;
                player.seasonStats.turnovers += playerStat.stats.turnovers;
                player.seasonStats.steals += (playerStat.stats.steals || 0);
                player.seasonStats.blocks += (playerStat.stats.blocks || 0);
            }
        });
    });
...
export const initializeEconomy = (team: Team): Team => {
    ...
    // Deduplicate Alumni (Fix for existing saves)
    if (updatedTeam.alumniRegistry?.allAlumni) {
        const uniqueAlumni = new Map<string, AlumniProfile>();
        updatedTeam.alumniRegistry.allAlumni.forEach(a => {
            if (!uniqueAlumni.has(a.id)) {
                uniqueAlumni.set(a.id, a);
            }
        });
        if (uniqueAlumni.size !== updatedTeam.alumniRegistry.allAlumni.length) {
            updatedTeam.alumniRegistry.allAlumni = Array.from(uniqueAlumni.values());
        }
    }
    ...
    const normalizedWealth = resolveWealth(updatedTeam);
    const finalizedWealth = { ...normalizedWealth };
    if (typeof finalizedWealth.boosterSentiment !== 'number') {
        finalizedWealth.boosterSentiment = clamp(50 + (finalizedWealth.donationLevel - 50) * 0.75, 0, 100);
    }
    if (typeof finalizedWealth.boosterLiquidity !== 'number') {
        finalizedWealth.boosterLiquidity = updatedTeam.warChest.discretionaryBudget || 0;
    }
    if (!finalizedWealth.alumniNetwork) {
        finalizedWealth.alumniNetwork = {
            strength: clamp(15 + finalizedWealth.donationLevel * 0.35 + finalizedWealth.boosterPool * 2, 0, 100),
            lastDonation: 0,
            lastDonationSeason: 0,
            lastDonationBreakdown: { nil: 0, facilities: 0, endowment: 0 },
        };
    }
    if (!Array.isArray(finalizedWealth.boosterReasons)) {
        finalizedWealth.boosterReasons = [];
    }
    updatedTeam.wealth = finalizedWealth;
...
/**
 * Improved retirement logic: Considers age, overall rating, and experience instead of just age.
 * - Age < 35: Never retire
 * - Age >= 42: Force retire
 * - Between: Chance increases with age and decreases with high overall
 */
export const applyNBAFreeAgentRetirementRules = (agents: NBAFreeAgent[]): NBAFreeAgent[] => {
    return agents.filter(agent => {
        ...
        // Never retire if under 35
        if (age < 35) return true;
        
        // Force retire at 42+
        if (age >= 42) return false;
        
        // Calculate retirement chance (increases with age, decreases with high overall)
        let retireChance = (age - 35) * 0.08; // 8% per year after 35
        
        // Low overall = more likely to retire
        if (overall < 72) retireChance += 0.15;
        if (overall < 65) retireChance += 0.25;
        
        // High overall = play longer (stars don't want to retire)
        if (overall >= 82) retireChance *= 0.5;
        if (overall >= 88) retireChance *= 0.3;
        
        // Long career fatigue
        if (experience >= 15) retireChance += 0.10;
        
        // Random chance to retire
...
/**
 * Generate domestic undrafted free agents (college seniors who went undrafted).
 * Lower ceiling but more NBA-ready due to 4 years of college.
 */
const generateUndraftedDomesticPlayers = (count: number): NBAFreeAgent[] => {
    return Array.from({ length: count }, () => {
        const forcedPosition = pickRandom(INTERNATIONAL_FREE_AGENT_POSITIONS);
        const player = createPlayer('Pro', forcedPosition);
        const targetOverall = randomBetween(64, 72); // Lower ceiling than international
        tunePlayerToOverall(player, targetOverall);
        player.age = randomBetween(22, 24); // College seniors
        player.experience = 0;
        player.collegeExperience = 4;
        player.contract = undefined;
        player.nbaStats = { gamesPlayed: 0, points: 0, rebounds: 0, assists: 0, minutes: 0 };
        player.originDescription = 'Undrafted Free Agent';
        player.proLearningCurve = {
            seasonsInNBA: 0,
            adaptationProgress: 40, // More NBA-ready due to college experience
            hasBreakoutSeason: false,
            peakReached: false,
        };
...
/**
 * Generate G-League/overseas returnee free agents.
 * Higher variance - some could be late bloomers, some are journeymen.
 */
const generateDevelopmentLeaguePlayers = (count: number): NBAFreeAgent[] => {
    return Array.from({ length: count }, () => {
        const forcedPosition = pickRandom(INTERNATIONAL_FREE_AGENT_POSITIONS);
        const player = createPlayer('Pro', forcedPosition);
        // Higher variance - could be diamond in rough or career end-of-bencher
        const targetOverall = randomBetween(65, 78);
        tunePlayerToOverall(player, targetOverall);
        player.age = randomBetween(24, 28);
        player.experience = randomBetween(1, 4);
        player.contract = undefined;
        player.nbaStats = { gamesPlayed: randomBetween(0, 82), points: 0, rebounds: 0, assists: 0, minutes: 0 };
        player.originDescription = pickRandom(['G-League', 'Overseas', 'Two-Way']);
        player.proLearningCurve = {
            seasonsInNBA: player.experience || 0,
            adaptationProgress: 60 + randomBetween(0, 30),
            hasBreakoutSeason: false,
            peakReached: player.age >= 28,
        };
...
/**
 * Generate initial NBA free agent pool with variety:
 * - International prospects (35): High ceiling raw talent
 * - Domestic undrafted (20): Lower ceiling but more polished
 * - G-League/overseas (15): High variance development players
 */
export const generateInitialNBAFreeAgents = (): NBAFreeAgent[] => {
    const agents: NBAFreeAgent[] = [];
    
    // International prospects (raw but high potential)
    agents.push(...generateInternationalFreeAgents(35));
    
    // Domestic undrafted (college seniors, more NBA-ready)
    agents.push(...generateUndraftedDomesticPlayers(20));
    
    // G-League/overseas returnees (high variance, some late bloomers)
    agents.push(...generateDevelopmentLeaguePlayers(15));
...
export const processNBAOffseasonDevelopment = (player: Player): Player => {
    ...
    const curve = player.proLearningCurve;
    const seasonsInNBA = curve?.seasonsInNBA || 0;
    
    // Age-based development curve (peak around 27-28)
    const ageMultiplier = (() => {
        if (age <= 22) return 1.4;       // Rapid growth for young players
        if (age <= 25) return 1.1;       // Strong growth
        if (age <= 28) return 0.6;       // Slowing growth
        if (age <= 31) return 0.0;       // Plateau
        if (age <= 34) return -0.5;      // Decline starts
        return -1.2;                      // Significant decline for older veterans
    })();
    
    // NBA Experience Curve (bigger jumps early, then plateau)
    const experienceMultiplier = (() => {
        if (seasonsInNBA === 0) return 1.5; // Rookie bump
        if (seasonsInNBA === 1) return 1.3; // Sophomore surge
        if (seasonsInNBA === 2) return 1.0; // Normal
        if (seasonsInNBA <= 4) return 0.7;  // Slowing gains
        return 0.3;                          // Veteran baseline
    })();
    
    // Potential-driven growth (can't exceed ceiling)
    const potentialFactor = clamp(upside / 15, 0, 1.5);
    
    // Calculate base change with randomness
    const rawChange = (potentialFactor * 3 + ageMultiplier) * experienceMultiplier;
    const jitter = randomBetween(-10, 10) / 10; // -1.0 to +1.0
    let change = Math.round(rawChange + jitter);
    
    // Ensure we don't exceed potential (with a small buffer)
    const cappedOverall = clamp(overall + change, 40, Math.min(99, potential + 2));
    const actualChange = cappedOverall - overall;
    ...
    const updated: Player = { 
        ...
        startOfSeasonOverall: cappedOverall,
    };
    
    // Contract year motivation boost: Players in final contract year perform better
    const contractYears = player.contract?.yearsLeft ?? 0;
    if (contractYears === 1 && !player.contractYearMotivated) {
        updated.contractYearMotivated = true;
        // Slight boost to scoring stats (contract year bump)
        const boostAmount = randomBetween(1, 2);
        updated.stats.outsideScoring = clamp(updated.stats.outsideScoring + boostAmount, 40, 99);
        updated.stats.insideScoring = clamp(updated.stats.insideScoring + boostAmount, 40, 99);
    }
    
    // Distribute the change across stats intelligently
    if (actualChange !== 0) {
        const statKeys: (keyof Player['stats'])[] = [
            'insideScoring', 'outsideScoring', 'playmaking',
            'perimeterDefense', 'insideDefense', 'rebounding', 'stamina'
        ];
        
        const stepsToApply = Math.abs(actualChange);
        const direction = actualChange > 0 ? 1 : -1;
        
        for (let i = 0; i < stepsToApply; i++) {
            // If improving, boost weakest stats; if declining, hit strongest first
            const sortedKeys = [...statKeys].sort((a, b) => 
                direction > 0 
                    ? updated.stats[a] - updated.stats[b]  // Improve weakest
                    : updated.stats[b] - updated.stats[a]  // Decline strongest
            );
            const statKey = sortedKeys[randomBetween(0, 2)]; // Some randomness in target stat
            updated.stats[statKey] = clamp(updated.stats[statKey] + direction, 40, 99);
        }
        
        // Recalculate overall based on new stats
        updated.overall = clamp(calculateOverall(updated.stats), 40, 99);
        updated.startOfSeasonOverall = updated.overall;
    }
    
    // Update pro learning curve
    const adaptationProgress = curve ? 
        clamp(curve.adaptationProgress + 10 + randomBetween(0, 10), 0, 100) : 
        Math.min(100, seasonsInNBA * 20 + randomBetween(10, 25));
    const hasBreakoutSeason = (curve?.hasBreakoutSeason || false) || 
        (seasonsInNBA >= 2 && adaptationProgress >= 75 && actualChange >= 4);
    const peakReached = updated.overall >= potential - 2 || age >= 30;
    
    updated.proLearningCurve = {
        seasonsInNBA: seasonsInNBA + 1,
        adaptationProgress,
        hasBreakoutSeason,
        peakReached,
    };
...
const processRecruitingDayActions = (
    ...
    const anchorProgramByState = new Map<string, { teamName: string; prestige: number }>();
    teams.forEach(team => {
        const state = normalizeStateCode(team.state || '');
        if (!state) return;
        const prestige = team.recruitingPrestige ?? team.prestige ?? 0;
        const current = anchorProgramByState.get(state);
        if (!current || prestige > current.prestige) {
            anchorProgramByState.set(state, { teamName: team.name, prestige });
        }
    });
    updatedRecruits.forEach(r => {
        r.isSigned = Boolean(r.isSigned || r.recruitmentStage === 'Signed' || r.signWeek != null);
        ...
        r.activeOfferCount = Number.isFinite(r.activeOfferCount)
            ? r.activeOfferCount
            : ((r.cpuOffers?.length || 0) + (r.userHasOffered ? 1 : 0));
        if (!r.hometownAnchorProgram) {
            const state = normalizeStateCode(r.hometownState || r.homeState || r.state || '');
            const anchor = anchorProgramByState.get(state);
            if (anchor) r.hometownAnchorProgram = anchor.teamName;
        }
        if (r.verbalCommitment && r.verbalDay == null) {
            r.verbalDay = r.commitWeek ?? recruitingWeek;
        }
        if (!r.verbalCommitment) {
            r.verbalLevel = 'NONE';
            r.lockStrength = 0;
    ...
    // Daily lock strength growth for verbally committed recruits (poachable until signed).
    updatedRecruits.forEach(r => {
        if (!r.verbalCommitment) return;
    ...
    const countActiveOffersForTeam = (teamName: string): number => {
        return updatedRecruits.filter(r => (r.cpuOffers || []).includes(teamName) && !r.isSigned).length;
    };

    const recordCpuOffer = (target: Recruit, team: Team) => {
        const prestige = team.recruitingPrestige ?? team.prestige ?? 50;
        const isDukeTier = prestige >= 94;
        if (target.isSigned) return;
        if (isDukeTier) {
            const recruitAcademicPreference = target.preferredProgramAttributes?.academics ?? 50;
            if (recruitAcademicPreference < 40 || target.personalityTrait === 'Family Feud') return;
        }
        if ((target.declinedOffers || []).includes(team.name)) return;
        if ((target.cpuOffers || []).includes(team.name)) return;
        if (target.verbalCommitment) {
            if (target.verbalCommitment === team.name) return;
            const committedTeam = teamsByName.get(target.verbalCommitment);
            const committedPrestige = committedTeam?.prestige ?? 50;
            const committedNil = committedTeam?.nilBudget ?? 0;
            const prestigeDelta = prestige - committedPrestige;
            const nilDelta = (team.nilBudget ?? 0) - committedNil;
            const significantUpgrade = prestigeDelta >= 6 || nilDelta >= 400000;
            if (!significantUpgrade) return;
            if (target.archetype === 'Mercenary' && nilDelta < 250000) return;
        }
    ...
    cpuTeams.forEach(team => {
        ...
        const outstanding = countActiveOffersForTeam(team.name);
        const capFactor = early ? 2.0 : mid ? 1.6 : late ? 1.2 : 1.0;
        const prestige = team.recruitingPrestige ?? team.prestige ?? 50;
        const maxActiveOffers = prestige >= 94 ? 18 : prestige >= 80 ? 25 : 40;
        const maxOutstanding = Math.min(
            maxActiveOffers,
            Math.max(2, Math.round(slotsRemaining * capFactor + (prestige >= 80 ? 1 : 0)))
        );
        ...
        if (outstanding > 0) {
            const offered = updatedRecruits
                .filter(r => (r.cpuOffers || []).includes(team.name) && !r.isSigned && r.verbalCommitment !== team.name)
                .map(r => {
        ...
        if (countActiveOffersForTeam(team.name) >= maxOutstanding) return;
        ...
        for (const target of offerCandidates) {
            ...
            if (countActiveOffersForTeam(team.name) >= maxOutstanding) break;
    ...
    // 3. Daily flip/decommit checks for verbals (poachable until signed).
    updatedRecruits.forEach(r => {
        const committedSchool = r.verbalCommitment;
        if (!committedSchool) return;
        if (r.isSigned) return;

        const offers = [...(r.cpuOffers || []), ...(r.userHasOffered ? [userTeamName] : [])]
            .filter(offer => !(r.declinedOffers || []).includes(offer));
        if (!offers.includes(committedSchool) && !(r.declinedOffers || []).includes(committedSchool)) {
            offers.push(committedSchool);
        }
        if (!offers.length) return;
        ...
        const offerDetails = offers
            ...
            .filter(Boolean) as { name: string; score: number }[];
        offerDetails.sort((a, b) => b.score - a.score);
        const leader = offerDetails[0];
        if (!leader) return;

        const shortlistForceInclude = [committedSchool, offerDetails[1]?.name].filter(Boolean) as string[];
        const { shares } = buildRecruitOfferShortlist(offerDetails, {
            min: 3,
            max: 6,
            leaderWindow: 10,
            forceInclude: shortlistForceInclude,
            seedKey: `${currentDateISO}:${r.id}:flip`,
            temperatureMultiplier: getRecruitOfferShareTemperatureMultiplier(r),
        });

        const committedScore = offerDetails.find(o => o.name === committedSchool)?.score ?? 0;
        const committedShare = shares.get(committedSchool) ?? 0;
        const leaderShare = shares.get(leader.name) ?? 0;
        const shareGap = leaderShare - committedShare;
        const scoreGap = leader.score - committedScore;

        if (leader.name !== committedSchool && shareGap > 15) {
            const delta = shareGap - 15;
            const sigmoid = 1 / (1 + Math.exp(-0.12 * delta));
            const flipChance = clamp((1 - clamp(r.lockStrength ?? 0, 0, 1)) * sigmoid, 0, 0.75);
            if (Math.random() < flipChance) {
                const from = committedSchool;
                const to = leader.name;
                r.verbalCommitment = to;
                r.verbalDay = recruitingWeek;
                r.lockStrength = clamp(0.12 + (r.lockStrength ?? 0) * 0.25, 0, 1);
                r.softCommitment = recruitingWeek <= 8;
                r.verbalLevel = r.softCommitment ? 'SOFT' : 'HARD';
                r.recruitmentStage = r.softCommitment ? 'SoftCommit' : 'HardCommit';
                r.lastRecruitingNews = `${r.name} flipped from ${from} to ${to}.`;
                r.isTargeted = false;
                if (to === userTeamName && r.userHasOffered) r.userHasOffered = false;
                logTelemetryEvent('FlipEvent', { recruitId: r.id, recruitName: r.name, from, to, week: recruitingWeek, shareGap, scoreGap, lockStrength: r.lockStrength });
                if (!r.recruitingEvents) r.recruitingEvents = [];
                r.recruitingEvents.push({ type: 'Flip', week: recruitingWeek, from, to });
                return;
            }
        }

        if (leader.name !== committedSchool && shareGap > 22 && recruitingWeek >= 4) {
            const resilience = clamp(r.resilience ?? 50, 0, 100);
            const softnessMultiplier = r.softCommitment ? 1.65 : 1.0;
            const resilienceMultiplier = clamp(1.15 - (resilience / 120), 0.25, 1.15);
            const baseChance = clamp((shareGap - 22) / 25, 0, 1) * 0.18;
            const chance = clamp(baseChance * softnessMultiplier * resilienceMultiplier, 0, 0.25);
            if (Math.random() < chance) {
                logTelemetryEvent('DecommitEvent', { recruitId: r.id, recruitName: r.name, from: committedSchool, week: recruitingWeek, shareGap, scoreGap, lockStrength: r.lockStrength ?? 0 });
                if (!r.recruitingEvents) r.recruitingEvents = [];
                r.recruitingEvents.push({ type: 'Decommit', week: recruitingWeek, from: committedSchool });
                r.lastRecruitingNews = `${r.name} reopened their recruitment after a late push from ${leader.name}.`;
                r.verbalCommitment = null;
                r.softCommitment = false;
                r.verbalLevel = 'NONE';
                r.lockStrength = 0;
...
export const runDailySimulation = (
    ...
    // Calculate day index for NBA simulation (days since NBA season start ~Oct 22)
    const calculateNBADayIndex = (dateISO: string): number => {
        const date = new Date(dateISO);
        // NBA season starts around Oct 22
        const seasonYear = date.getMonth() >= 9 ? date.getFullYear() : date.getFullYear() - 1;
        const nbaSeasonStart = new Date(seasonYear, 9, 22); // October 22nd
        const daysSinceStart = Math.floor((date.getTime() - nbaSeasonStart.getTime()) / (1000 * 60 * 60 * 24));
        return Math.max(0, daysSinceStart); // Don't go negative if before season start
    };
    
    // Process NBA games in the background (runs regardless of college mode)
    const nbaDayIndex = calculateNBADayIndex(currentDate);
    const nbaUpdates = processDailyNBAGames(state, nbaDayIndex);
...
// --- RECRUITING SYSTEM V2 HELPERS ---

// 1. Rating Functions
const getTeamAcademicScore_Legacy = (team: Team): number => {
    // Academics based on prestige (proxy) and institutional data if available
    if (team.institutionalProfile?.prestigeAcademicBase) {
        return team.institutionalProfile.prestigeAcademicBase; // 0-100 real data
    }
    // Fallback: Elite schools usually have decent academics, low majors vary
    // Duke/Stanford/IVY logic would be better with real data, but linear prestige correlation is a safe default
    return Math.min(100, Math.max(20, team.prestige * 0.8 + 20));
};

const getTeamMarketScore_Legacy = (team: Team): number => {
    if (team.institutionalProfile?.marketSizeScore) {
        return team.institutionalProfile.marketSizeScore;
    }
    // Default fallback based on prestige (bigger schools usually in better markets/TV slots)
    return team.prestige;
};

const getTeamCommunityScore_Legacy = (team: Team): number => {
    // Fan interest is best proxy
    return team.fanInterest;
};

const getTeamDevelopmentScore_Legacy = (team: Team): number => {
    // Combo of Facilities + Coach Grades
    const facilityScore = (
        (team.facilities?.training?.level || 1) + 
        (team.facilities?.coaching?.level || 1)
    ) / 10 * 100; // max 100

    const coachDevGrade = team.headCoach.grades?.development || 'C';
    const gradeScores: Record<string, number> = { 'A': 100, 'B': 85, 'C': 70, 'D': 55, 'F': 40 };
    const coachScore = gradeScores[coachDevGrade] || 70;

    return (facilityScore * 0.4) + (coachScore * 0.6);
};

const getTeamExposureScore_Legacy = (team: Team): number => {
    // Prestige + Conference + Recent Success
    let score = team.prestige;
    if (['ACC', 'SEC', 'Big 10', 'Big 12', 'Big East'].includes(team.conference)) score += 10;
    if (team.record.wins > 20) score += 10;
    return Math.min(100, score);
};

const getTeamNILCompetitiveness_Legacy = (team: Team, recruit: Recruit | Transfer): number => {
    // Compare offer/budget vs expectation
    const budget = team.nilBudget || (team.prestige * 25000);
    const expectation = recruit.nilValue || 50000; // default assumption
    const ratio = budget / (expectation * 20); // Can we afford 20 of these guys?
    // Scale 0-100. If we have 20x expectation in budget, we are very competitive (100)
    // If we have 1x expectation, we are broke (5)
    return Math.min(100, Math.max(0, ratio * 100));
};

const getTeamProximityToRecruit_Legacy = (team: Team, recruit: Recruit): number => {
    // 0 = 3000 miles, 100 = 0 miles
    // Using Haversine
    if (!recruit.hometownLat || !recruit.hometownLon || !team.location) return 50; // Unknown
    
    // Quick inline Haversine
    const R = 3959; // miles
    const dLat = (team.location.lat - recruit.hometownLat) * Math.PI / 180;
    const dLon = (team.location.lon - recruit.hometownLon) * Math.PI / 180;
    const a = Math.sin(dLat/2) * Math.sin(dLat/2) +
              Math.cos(recruit.hometownLat * Math.PI / 180) * Math.cos(team.location.lat * Math.PI / 180) *
              Math.sin(dLon/2) * Math.sin(dLon/2);
    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
    const dist = R * c;

    // Linear drop off: 100 at 0 miles, 0 at 2000 miles
    return Math.max(0, 100 - (dist / 20));
};

const getTeamPlayingTimeScore_Legacy = (team: Team, recruit: Recruit | Transfer): number => {
    // Check depth chart at position
    const pos = recruit.position;
    const playersAtPos = team.roster.filter(p => p.position === pos).length;
    // Simple logic: 0 players = 100, 1 player = 80, 2 = 60, 3+ = 20
    if (playersAtPos === 0) return 100;
    if (playersAtPos === 1) return 80;
    if (playersAtPos === 2) return 60;
    return 30;
};

// 2. Staff Generation
const generateWeightedGrade = (prestige: number): StaffGrade => {
    const r = Math.random() * 100;
    if (prestige >= 90) {
        // Elite: 40% A, 35% B, 20% C, 5% D
        if (r < 40) return 'A';
        if (r < 75) return 'B';
        if (r < 95) return 'C';
        return 'D';
    } else if (prestige >= 70) {
        // High Major: 20% A, 35% B, 30% C, 15% D
        if (r < 20) return 'A';
        if (r < 55) return 'B';
        if (r < 85) return 'C';
        return 'D';
    } else if (prestige >= 50) {
        // Mid Major: 10% A, 25% B, 35% C, 30% D
        if (r < 10) return 'A';
        if (r < 35) return 'B';
        if (r < 70) return 'C';
        return 'D';
    } else {
        // Low Major: 5% A, 15% B, 30% C, 50% D
        if (r < 5) return 'A';
        if (r < 20) return 'B';
        if (r < 50) return 'C';
        return 'D';
...
export const createHeadCoach = (prestige: number): HeadCoachProfile => {
    const grade = generateWeightedGrade(prestige);
    // Correlate reputation with grade
    const repMap: Record<StaffGrade, number> = { 'A': 90, 'B': 75, 'C': 50, 'D': 30 };
    
    return {
        name: generatePlayerName(), // Use player name pool for coaches too
        age: 40 + Math.floor(Math.random() * 25),
        almaMater: 'Unknown University',
        style: pickRandom(['Uptempo', 'Defensive', 'Balanced', 'Motion', 'Recruiting', 'Player Development']),
        reputation: repMap[grade] + Math.floor(Math.random() * 10),
        seasons: Math.floor(Math.random() * 20),
        careerWins: 0,
        careerLosses: 0,
        seasonWins: 0,
        seasonLosses: 0,
        startSeason: 2025 - Math.floor(Math.random() * 10),
        careerStops: [],
        grades: {
            // HCs usually spike in one area based on style, but we'll generate based on prestige for now
            recruiting: generateWeightedGrade(prestige),
            development: generateWeightedGrade(prestige),
            gameday: generateWeightedGrade(prestige),
            relationships: generateWeightedGrade(prestige),
        }
    };
};

export const ensureFullTeamData = (team: Team) => {
    // 1. Ensure HC
    if (!team.headCoach || !team.headCoach.grades) {
        // Re-generate if missing or upgrading legacy
        team.headCoach = createHeadCoach(team.prestige);
    }

    // 2. Ensure Assistants
    if (!team.staff.assistants || team.staff.assistants.length < 3) {
        const needed = 3 - (team.staff.assistants?.length || 0);
        const newAssistants = Array.from({ length: needed }, () => {
           const s = createStaff('Assistant Coach'); 
           s.grade = generateWeightedGrade(team.prestige); 
           return s;
        });
        team.staff.assistants = [...(team.staff.assistants || []), ...newAssistants];
    }

    // 2b. Ensure Trainers/Scouts
    if (!team.staff.trainers || team.staff.trainers.length < 2) {
         const needed = 2 - (team.staff.trainers?.length || 0);
         const newTrainers = Array.from({ length: needed }, () => createStaff('Trainer'));
         team.staff.trainers = [...(team.staff.trainers || []), ...newTrainers];
    }
    if (!team.staff.scouts || team.staff.scouts.length < 2) {
         const needed = 2 - (team.staff.scouts?.length || 0);
         const newScouts = Array.from({ length: needed }, () => createStaff('Scout'));
         team.staff.scouts = [...(team.staff.scouts || []), ...newScouts];
    }
    
    // 3. Ensure Facilities (Legacy fill)
    if (!team.facilities) {
        // Create basic facilities
        const level = Math.max(1, Math.floor(team.prestige / 20));
        team.facilities = {
            arena: { name: `${team.name} Arena`, capacity: 5000, luxurySuites: 0, seatMix: {} as any, attendanceLog: [], level, quality: 50, maintenanceCost: 10000 },
            training: { level, quality: 50, maintenanceCost: 5000, equipmentLevel: level } as any,
            medical: { level, quality: 50, maintenanceCost: 5000 } as any,
            scouting: { level, quality: 50, maintenanceCost: 5000, networkReach: team.prestige } as any,
            coaching: { level, quality: 50, maintenanceCost: 5000, technologyLevel: level } as any,
        };
    }
    ...
    if (!team.nilBudget) {
        ...
        team.nilBudget = Math.floor(team.prestige * 25000 * variance);
    }
};

// 3. Recruiting Context & Daily Processing
export interface RecruitingContext {
    gameInSeason: number;
    currentDate: string; // ISO
    isSigningPeriod: boolean;
    recentGameResults: Map<string, 'W' | 'L'>; // teamName -> result
    newsEvents: TimelineEvent[]; // For momentum
    commitsTodayCount: number; // For global throttle
    allTeams: Team[];
}

// ============================================
// TEAM RATING FUNCTIONS (0-100 scale)
// Required per longprompt.txt specifications
// ============================================

/**
 * Academic score based on institutional profile or prestige fallback
 */
export const getTeamAcademicScore = (team: Team): number => {
    if (team.institutionalProfile?.prestigeAcademicFinal != null) {
        return Math.min(100, Math.max(0, team.institutionalProfile.prestigeAcademicFinal));
    }
    // Fallback: prestige-based approximation
    return Math.min(100, Math.max(0, team.prestige * 0.7 + 15));
};

/**
 * Market score based on city size, TV market, or revenue as proxy
 */
export const getTeamMarketScore = (team: Team): number => {
    // Use finances as proxy for market size
    if (team.finances?.baseRevenue) {
        // Scale: $500K = 50, $2M = 100
        return Math.min(100, Math.max(0, (team.finances.baseRevenue / 20000)));
    }
    // Fallback: prestige-based
    return Math.min(100, Math.max(0, team.prestige * 0.8));
};

/**
 * Community engagement based on fan interest, arena attendance, alumni support
 */
export const getTeamCommunityScore = (team: Team): number => {
    let score = 50; // Base
    
    if (team.fanInterest != null) {
        score = team.fanInterest;
    }
    
    // Boost from alumni wealth if available
    const wealthBonus: Record<string, number> = { 'Comfortable': 5, 'Wealthy': 10, 'Elite': 20 };
    const wealthScore = typeof team.wealth === 'string'
        ? (team.wealth !== 'Modest' ? wealthBonus[team.wealth] || 0 : 0)
        : Math.round(resolveWealth(team).donationLevel / 20);
    score += wealthScore;
...
/**
 * Development score based on facilities level + coaching grades
 */
export const getTeamDevelopmentScore = (team: Team): number => {
    let score = 50;
    
    // Facilities contribution (0-50 points)
    if (team.facilities) {
        const avgLevel = (
            (team.facilities.training?.level || 1) +
            (team.facilities.medical?.level || 1) +
            (team.facilities.coaching?.level || 1)
        ) / 3;
        score = avgLevel * 10; // Level 1-5 -> 10-50
    }
    
    // Coaching grade contribution (0-50 points)
    if (team.headCoach?.grades?.development) {
        const gradeValue: Record<string, number> = { 'A': 50, 'B': 40, 'C': 30, 'D': 20, 'F': 10 };
        score += gradeValue[team.headCoach.grades.development] || 25;
    } else {
        score += 25; // Default if no coach
    }
...
/**
 * Exposure score based on prestige + conference strength + market
 */
export const getTeamExposureScore = (team: Team): number => {
    let score = team.prestige * 0.5; // Prestige is 50% of exposure
    
    // Conference strength bonus
    const confStrength = team.conferenceStrength || 'Mid';
    const confBonus: Record<string, number> = { 'Elite': 25, 'High': 20, 'Mid': 15, 'Low': 10 };
    score += confBonus[confStrength] || 15;
    
    // Market contribution
    score += getTeamMarketScore(team) * 0.25;
    
    return Math.min(100, Math.max(0, score));
};

/**
 * NIL competitiveness based on team budget vs recruit expectation
 */
export const getTeamNILCompetitiveness = (team: Team, recruit: Recruit): number => {
    const teamBudget = team.nilBudget || team.prestige * 25000;
    
    // Recruit expectation based on stars
    const starExpectation: Record<number, number> = {
        5: 500000,
        4: 200000,
        3: 75000,
        2: 25000,
    ...
    const recruitExpectation = starExpectation[recruit.stars] || 50000;
    
    // Calculate score: budget / expectation * 100, capped
    const ratio = teamBudget / recruitExpectation;
    return Math.min(100, Math.max(0, ratio * 50)); // 2x budget = 100 score
};

/**
 * Proximity score - inverse of distance (closer = higher score)
 */
export const getTeamProximityToRecruit = (team: Team, recruit: Recruit): number => {
    // Try to get distance
    const distanceMiles = estimateRecruitDistanceMilesToTeam(recruit, team);
    
    if (distanceMiles === null || distanceMiles === undefined) {
        // Check if same state
        if (recruit.homeState && team.state && recruit.homeState === team.state) {
            return 85; // Same state = high proximity
        }
        return 50; // Unknown distance = neutral
    }
    
    // Convert distance to score: 0 miles = 100, 3000+ miles = 0
    const score = 100 - Math.min(100, distanceMiles / 30);
    return Math.max(0, score);
};

/**
 * Playing time score based on roster depth at recruit's position
 */
export const getTeamPlayingTimeScore = (team: Team, recruit: Recruit): number => {
    if (!team.roster || team.roster.length === 0) {
        return 80; // No roster = lots of playing time available
    }
    
    const position = recruit.position;
    
    // Count players at same position
    const samePositionPlayers = team.roster.filter(p => 
        p.position === position || p.secondaryPosition === position
    );
    
    // Score based on depth: 0 players = 100, 3+ = 40
    const depthPenalty = Math.min(60, samePositionPlayers.length * 20);
    let score = 100 - depthPenalty;
    
    // Boost if starters at position are seniors (graduating)
    const seniorStarters = samePositionPlayers.filter(p => 
        p.year === 'Sr' && p.starterPosition === position
    ).length;
    score += seniorStarters * 15;
    
    return Math.min(100, Math.max(0, score));
};

const getTeamAttributes = (team: Team): ProgramPreference => {
    // Helper to extract comparable attributes from a Team object
    // normalized 0-100
    return {
        academics: team.institutionalProfile?.prestigeAcademicFinal ?? 50,
        marketExposure: Math.min(100, (team.finances.baseRevenue / 1000000) * 2), // Rough proxy
        communityEngagement: team.fanInterest ?? 50
    };
};

const calculateInterestScore = (recruit: Recruit, team: Team, context: RecruitingContext): number => {
    // 1. Base Prestige Match
    // 5-star (95+) wants 90+ prestige. 3-star (75) is happy with 60.
    const prestigeDiff = team.prestige - recruit.stars * 20; // e.g. 90 - 100 = -10. 
    let baseScore = 0;
    
    // Penalize punching too high, but allow "sleeping giants"
    if (prestigeDiff < -20) baseScore = 10;
    else if (prestigeDiff < -10) baseScore = 40;
    else baseScore = 60 + prestigeDiff; // 90 team vs 5 star (100 equiv) -> 50 base. 

    // 2. Proximity
    let proximityBonus = 0;
    if (team.state === recruit.homeState) proximityBonus = 25;
    else if (recruit.region === 'Midwest' && ['IL','IN','OH','MI'].includes(team.state)) proximityBonus = 10; // Simplified region logic
    
    // 3. Fit
    const teamAttrs = getTeamAttributes(team);
    const fitScore = (
        (100 - Math.abs(recruit.preferredProgramAttributes.academics - teamAttrs.academics)) +
        (100 - Math.abs(recruit.preferredProgramAttributes.marketExposure - teamAttrs.marketExposure)) + 
        (100 - Math.abs(recruit.preferredProgramAttributes.communityEngagement - teamAttrs.communityEngagement))
    ) / 3;
    const fitBonus = fitScore * 0.3; // Weight fit at 30%

    // 4. Offers
    let offerBonus = 0;
    if (recruit.cpuOffers.includes(team.name) || (recruit.userHasOffered && team.isUserTeam)) {
        offerBonus = 30; // Huge boost for offers
    }

    // 5. Random Jitter
    const jitter = (Math.random() * 4) - 2;

    return Math.max(0, Math.min(100, baseScore + proximityBonus + fitBonus + offerBonus + jitter));
};

const recalculateRecruitInterestScores = (recruit: Recruit, context: RecruitingContext): Record<string, number> => {
    const scores: Record<string, number> = { ...(recruit.interestMap || {}) };
    
    // Optimization: Only score "relevant" teams to avoid 350x3000 loops.
    // Relevant = Already in interestMap, OR Top Prestige, OR Local, OR Offering.
    
    // 1. Ensure Blue Bloods are considered for 4/5 stars (Prestige Forcing)
    if (recruit.stars >= 4) {
        context.allTeams.filter(t => t.prestige >= 90).forEach(t => {
            if (scores[t.name] === undefined) scores[t.name] = 50; // Seed if missing
        });
    }

    // 2. Ensure Local Teams are considered
    if (recruit.homeState) {
         context.allTeams.filter(t => t.state === recruit.homeState).forEach(t => {
            if (scores[t.name] === undefined) scores[t.name] = 40; // Seed
        });
    }

    // 3. Score all currently tracked teams
    Object.keys(scores).forEach(teamName => {
        const team = context.allTeams.find(t => t.name === teamName);
        if (!team) return;
        
        // Decay existing slightly before recalc? Or just recalc distinct.
        // Let's recalc fresh based on static factors + dynamic momentum.
        const newScore = calculateInterestScore(recruit, team, context);
        
        // Apply momentum from teamMomentum tracking
        const momentum = recruit.teamMomentum?.[teamName] || 0;
        const momentumBonus = clamp(momentum * 0.5, -15, 15); // Half of momentum as score bonus
        const adjustedNewScore = clamp(newScore + momentumBonus, 0, 100);
        
        // 50/50 blend for more responsive changes (was 80/20)
        scores[teamName] = Math.round((scores[teamName] + adjustedNewScore) / 2);
    });

    return scores;
};

const rebuildShortlists = (recruit: Recruit, interestMap: Record<string, number>): Recruit => {
    // Sort teams by interest
    const sortedTeams = Object.entries(interestMap)
        .sort(([, a], [, b]) => b - a)
        .slice(0, 10); // Keep top 10
    
    // Reconstruct map with only top 10 to save memory/space
    const newMap: Record<string, number> = {};
    sortedTeams.forEach(([team, score]) => newMap[team] = score);
...
const checkFormalDeclines = (recruit: Recruit, context: RecruitingContext): { updatedRecruit: Recruit, events: TimelineEvent[] } => {
    let updated = { ...recruit };
    const events: TimelineEvent[] = [];
    
    // Sort interest map
    const sortedTeams = Object.entries(updated.interestMap)
        .sort(([, a], [, b]) => b - a);
    
    if (sortedTeams.length === 0) return { updatedRecruit: updated, events };
    
    const leaderScore = sortedTeams[0][1];
    const threshold = leaderScore * 0.7; // Cutoff: 70% of leader's score
    
    // Identify teams to drop
    const teamsToDrop: string[] = [];
    
    Object.entries(updated.interestMap).forEach(([teamName, score]) => {
        // Can't decline if offered? Or can? Realistically, yes.
        // Don't decline the leader even if score is low (early cycle)
        if (teamName === sortedTeams[0][0]) return;

        if (score < threshold && score < 40) { // Absolute floor of 40 to stay alive
             teamsToDrop.push(teamName);
        }
    });

    if (teamsToDrop.length > 0) {
        const newMap = { ...updated.interestMap };
        teamsToDrop.forEach(t => {
            delete newMap[t];
            // Log event if it was a relevant team (e.g., user or high prestige)
            const team = context.allTeams.find(tm => tm.name === t);
            if (team && (team.isUserTeam || team.prestige >= 85)) {
                events.push({
                    type: 'Decline',
                    week: context.gameInSeason, // Using gameInSeason as proxy for week, though ideally we have 'week'
                    team: t,
                    message: `${recruit.name} has eliminated ${t} from consideration.`,
                    date: context.currentDate,
                    recruitId: recruit.id
                });
            }
        });
        updated.interestMap = newMap;
    }
    
    return { updatedRecruit: updated, events };
};

/**
 * Enhanced decline logic using the 5-day longshot rule from longprompt.txt
 */
const checkFormalDeclinesEnhanced = (recruit: Recruit, context: RecruitingContext): { updatedRecruit: Recruit, events: TimelineEvent[] } => {
    let updated = { ...recruit };
    const events: TimelineEvent[] = [];
    ...
    const teamsToDrop: string[] = [];
    const longshotHistory = updated.longshotHistory || {};
    const shortlistHistory = updated.shortlistHistory || {};
    
    // Calculate total interest for share calculation
    const totalInterest = Object.values(updated.interestMap).reduce((a, b) => a + b, 0) || 1;
    
    Object.entries(updated.interestMap).forEach(([teamName, score]) => {
        // Don't decline the leader
        if (teamName === sortedTeams[0]?.[0]) return;
        
        const share = (score / totalInterest) * 100;
        const daysAsLongshot = longshotHistory[teamName] || 0;
        const daysOnShortlistWithLowShare = shortlistHistory[teamName] && share < 5 ? shortlistHistory[teamName] : 0;
        
        // Decline conditions (per longprompt.txt):
        // 1. 5+ consecutive days as longshot
        // 2. 5+ consecutive days on shortlist with <5% share
        if (daysAsLongshot >= 5 || daysOnShortlistWithLowShare >= 5) {
            teamsToDrop.push(teamName);
        }
    });
    ...
    if (teamsToDrop.length > 0) {
        const newMap = { ...updated.interestMap };
        const newShortlistHistory = { ...updated.shortlistHistory };
        const newLongshotHistory = { ...updated.longshotHistory };
        
        teamsToDrop.forEach(t => {
            delete newMap[t];
            delete newShortlistHistory[t];
            delete newLongshotHistory[t];
            
            // Add to declined offers
            if (!updated.declinedOffers.includes(t)) {
                updated.declinedOffers = [...updated.declinedOffers, t];
            }
            
            // Log event if it was a relevant team (user or high prestige)
            const team = context.allTeams.find(tm => tm.name === t);
            if (team && (team.isUserTeam || team.prestige >= 80)) {
                events.push({
                    type: 'Decline',
                    week: context.gameInSeason,
                    team: t,
                    message: `${recruit.name} has formally declined ${t}.`,
                    date: context.currentDate,
                    recruitId: recruit.id,
                    importance: team.isUserTeam ? 90 : 50
                });
            }
        });
        
        updated.interestMap = newMap;
        updated.shortlistHistory = newShortlistHistory;
        updated.longshotHistory = newLongshotHistory;
    }
    
    return { updatedRecruit: updated, events };
};

const checkForCommitment = (recruit: Recruit, context: RecruitingContext): { updatedRecruit: Recruit, events: TimelineEvent[] } => {
    let updated = { ...recruit };
    const events: TimelineEvent[] = [];
    
    if (updated.verbalCommitment) return { updatedRecruit: updated, events };
    if (context.commitsTodayCount >= 5) return { updatedRecruit: updated, events }; // Throttle

    // Build offer details for shortlist calculation
    const offerNames = [
        ...updated.cpuOffers,
        ...(updated.userHasOffered ? [context.allTeams.find(t => t.isUserTeam)?.name].filter(Boolean) as string[] : [])
    ];
    
    if (offerNames.length < 1) return { updatedRecruit: updated, events };
    
    const offerDetails = offerNames.map(teamName => {
        const score = updated.interestMap[teamName] || 0;
        const team = context.allTeams.find(t => t.name === teamName);
        return { name: teamName, score, prestige: team?.prestige ?? 50 };
    }).filter(o => o.score > 0);
    
    if (offerDetails.length < 1) return { updatedRecruit: updated, events };
    
    // Calculate shares using the same logic as the UI
    const { shares } = buildRecruitOfferShortlist(offerDetails, { 
        min: 3, 
        max: 6, 
        leaderWindow: 10, 
        seedKey: `${recruit.id}:${context.gameInSeason}`,
        temperatureMultiplier: getRecruitOfferShareTemperatureMultiplier(recruit)
    });
    
    // Sort by share percentage to find true leader
    const sortedByShare = [...shares.entries()].sort((a, b) => b[1] - a[1]);
    
    if (sortedByShare.length < 1) return { updatedRecruit: updated, events };
    
    const [leaderName, leaderShare] = sortedByShare[0];
    const runnerUpShare = sortedByShare.length > 1 ? sortedByShare[1][1] : 0;
    
    // Store current leader/second on recruit for UI consistency
    updated.currentLeader = leaderName;
    updated.currentSecond = sortedByShare[1]?.[0] ?? null;
    updated.leaderShare = leaderShare;
    
    // Check offer exists for leader
    const leaderTeam = context.allTeams.find(t => t.name === leaderName);
    const hasOffer = recruit.cpuOffers.includes(leaderName) || (leaderTeam?.isUserTeam && recruit.userHasOffered);
    
    if (!hasOffer) return { updatedRecruit: updated, events };

    // Commitment thresholds based on SHARES (%), not raw scores
    // Leader needs >= 35% share AND >= 10% gap over runner-up
    let shareThreshold = 35;
    let gapThreshold = 10;
    
    // Archetype Modifiers
    if (recruit.archetype === 'Mercenary') {
        shareThreshold = 40; // More demanding
    } else if (recruit.archetype === 'HometownHero' && leaderTeam?.state === recruit.homeState) {
        shareThreshold = 30; // Easier for hometown
        gapThreshold = 7;
    } else if (recruit.archetype === 'Loyalist') {
        shareThreshold = 32; // Loyal to early leader
    }
    
    // Days-based modifier: longer wait = more likely to commit
    const daysBonus = Math.min((recruit.daysInRecruitingCycle || 0) * 0.5, 10);
    const effectiveThreshold = Math.max(25, shareThreshold - daysBonus);
    
    // Commit chance scales with decisiveness of lead
    const shareGap = leaderShare - runnerUpShare;
    const baseCommitChance = 0.08; // 8% base
    const gapBonus = Math.max(0, (shareGap - gapThreshold) * 0.02); // +2% per point above threshold
    const commitChance = Math.min(0.35, baseCommitChance + gapBonus); // Cap at 35%

    const roll = Math.random();
    
    if (leaderShare >= effectiveThreshold && shareGap >= gapThreshold && roll < commitChance) {
        updated.verbalCommitment = leaderName;
        updated.softCommitment = true;
        updated.verbalDay = context.gameInSeason; 

        // Hard commit check (dominant lead)
        if (leaderShare > 50 && shareGap > 20) {
            updated.lockStrength = 1.0;
            updated.softCommitment = false; // Is hard
        } else {
            updated.lockStrength = 0.5; // Soft
        }

        context.commitsTodayCount++;

        events.push({
            type: 'Commit',
            week: context.gameInSeason,
            team: leaderName,
            message: `${recruit.name} has committed to ${leaderName}!`,
            date: context.currentDate,
            recruitId: recruit.id
        });
    }
...
export const processRecruitingDay = (
    recruits: Recruit[], 
    context: RecruitingContext
): { updatedRecruits: Recruit[], events: TimelineEvent[] } => {
    let events: TimelineEvent[] = [];
    const userTeam = context.allTeams.find(t => t.isUserTeam);
    
    // Iterate and update recruits
    const updatedRecruits = recruits.map(recruit => {
        // Skip if already signed
        if (recruit.isSigned) return recruit;

        let updated = { ...recruit };
        const recruitEvents: TimelineEvent[] = [];
        
        // 1. Update Lifecycle Counters
        updated.daysInRecruitingCycle = (updated.daysInRecruitingCycle || 0) + 1;
        
        // 2. Interest Recalculation & Shortlists
        const newInterestMap = recalculateRecruitInterestScores(updated, context);
        let withShortlist = rebuildShortlists({ ...updated, interestMap: newInterestMap }, newInterestMap);
        
        // 3. Update Shortlist/Longshot History
        const shortlistTeams = new Set(Object.keys(withShortlist.interestMap).slice(0, 6));
        const newShortlistHistory: Record<string, number> = {};
        const newLongshotHistory: Record<string, number> = {};
        
        // Update shortlist history
        for (const teamName of Object.keys(withShortlist.interestMap)) {
            if (shortlistTeams.has(teamName)) {
                // Team is on shortlist - increment counter, reset longshot
                newShortlistHistory[teamName] = (updated.shortlistHistory?.[teamName] || 0) + 1;
            } else {
                // Team is longshot - increment longshot counter
                newLongshotHistory[teamName] = (updated.longshotHistory?.[teamName] || 0) + 1;
            }
        }
        withShortlist.shortlistHistory = newShortlistHistory;
        withShortlist.longshotHistory = newLongshotHistory;
        
        // 4. Share Shift Detection (≥10% change)
        const oldSnapshot = updated.lastShareSnapshot || {};
        const totalInterest = Object.values(withShortlist.interestMap).reduce((a, b) => a + b, 0) || 1;
        const newSnapshot: Record<string, number> = {};
        
        for (const [teamName, score] of Object.entries(withShortlist.interestMap)) {
            const share = (score / totalInterest) * 100;
            newSnapshot[teamName] = share;
            
            const oldShare = oldSnapshot[teamName] || 0;
            const delta = share - oldShare;
            
            // Log significant shifts (only for user team or top schools)
            if (Math.abs(delta) >= 10) {
                ...
                if (team && (team.isUserTeam || team.prestige >= 80)) {
                    recruitEvents.push({
                        ...
                        week: context.gameInSeason,
                        team: teamName,
                        message: `${teamName}'s stock is ${delta > 0 ? 'rising' : 'falling'}. (${delta > 0 ? '+' : ''}${delta.toFixed(0)}%)`,
                        date: context.currentDate,
        ...
        withShortlist.lastShareSnapshot = newSnapshot;
        
        // 5. Enhanced Formal Declines (5-day longshot rule)
        const declineResult = checkFormalDeclinesEnhanced(withShortlist, context);
        withShortlist = declineResult.updatedRecruit;
        recruitEvents.push(...declineResult.events);

        // 6. Check Commitment (only after grace period)
        if (updated.daysInRecruitingCycle >= 7) {
            const commitResult = checkForCommitment(withShortlist, context);
            withShortlist = commitResult.updatedRecruit;
            recruitEvents.push(...commitResult.events);
        }

        // Sync legacy 'interest' field for User Team
        if (userTeam && withShortlist.interestMap[userTeam.name] !== undefined) {
            withShortlist.interest = withShortlist.interestMap[userTeam.name];
        }

        events.push(...recruitEvents);
        return withShortlist;
    });

    return { updatedRecruits, events };
};

// ============================================
// PHASE 3: CPU RECRUITING BEHAVIOR
// ============================================

/**
 * Calculate CPU team's daily recruiting budget based on prestige
 */
const getCPUDailyBudget = (team: Team): number => {
    if (team.prestige >= 90) return 15;
    if (team.prestige >= 50) return 10;
    return 5;
};

/**
 * Calculate recruit priority for CPU team targeting
 * Priority = (FitScore × 0.6) + (PositionNeed × 0.3) + (StarRating × 0.1)
 */
const getRecruitPriorityForCPU = (team: Team, recruit: Recruit): number => {
    // FitScore: How well does this recruit match the team's program?
    const teamAttrs = {
        academics: getTeamAcademicScore(team),
        marketExposure: getTeamExposureScore(team),
        communityEngagement: getTeamCommunityScore(team)
    };
    
    let fitScore = 0;
    const recruitPrefs = recruit.preferredProgramAttributes;
    if (recruitPrefs) {
        fitScore = (
            Math.abs(teamAttrs.academics - recruitPrefs.academics) +
            Math.abs(teamAttrs.marketExposure - recruitPrefs.marketExposure) +
            Math.abs(teamAttrs.communityEngagement - recruitPrefs.communityEngagement)
        ) / 3;
        fitScore = 100 - fitScore; // Invert: smaller diff = higher fit
    } else {
        fitScore = 50; // Neutral if no prefs
    }
    
    // PositionNeed: Does the team need this position?
    let positionNeed = 50;
    if (team.roster) {
        const samePosition = team.roster.filter(p => p.position === recruit.position).length;
        positionNeed = Math.max(0, 100 - samePosition * 25); // 0 = 100, 4+ = 0
        
        // Boost if seniors at position
        const seniors = team.roster.filter(p => p.position === recruit.position && p.year === 'Sr').length;
        positionNeed += seniors * 15;
    }
    positionNeed = Math.min(100, positionNeed);
    
    // StarRating contribution
    const starScore = recruit.stars * 20; // 5-star = 100, 1-star = 20
    
    return (fitScore * 0.6) + (positionNeed * 0.3) + (starScore * 0.1);
};

/**
 * Process CPU team recruiting actions for one day
 */
export const processCPURecruitingDay = (
    cpuTeams: Team[],
    recruits: Recruit[],
    context: RecruitingContext
): { updatedRecruits: Recruit[], events: TimelineEvent[] } => {
    ...
    for (const team of cpuTeams) {
        if (team.isUserTeam) continue; // Skip user team
        
        const budget = getCPUDailyBudget(team);
        let pointsSpent = 0;
        
        // Get uncommitted recruits sorted by priority
        const targets = recruits
            .filter(r => !r.isSigned && !r.verbalCommitment)
            .map(r => ({ recruit: r, priority: getRecruitPriorityForCPU(team, r) }))
            .sort((a, b) => b.priority - a.priority);
        
        for (const { recruit } of targets) {
            if (pointsSpent >= budget) break;
            
            const updated = recruitsById.get(recruit.id)!;
            const hasOffer = updated.cpuOffers.includes(team.name);
            const hasDeclined = updated.declinedOffers.includes(team.name);
            
            if (hasDeclined) continue; // Can't recruit if declined
            
            // Decision: Offer or Contact?
            if (!hasOffer) {
                // Should we offer?
                const shouldOffer = 
                    recruit.stars >= 3 || // Always try for 3+ stars
                    (recruit.stars >= 2 && getRecruitPriorityForCPU(team, recruit) > 60) || // Good fit 2-star
                    (getTeamProximityToRecruit(team, recruit) > 70); // Local recruit
                
                if (shouldOffer && pointsSpent + 3 <= budget) {
                    // Extend offer (3 points)
                    updated.cpuOffers = [...updated.cpuOffers, team.name];
                    updated.activeOfferCount = (updated.activeOfferCount || 0) + 1;
                    pointsSpent += 3;
                    ...
                    allEvents.push({
                        ...
                        week: context.gameInSeason,
                        team: team.name,
                        message: `${team.name} extended a scholarship offer.`,
                        date: context.currentDate,
                        recruitId: updated.id
                    });
                    
                    // Initialize interest if needed
                    if (!updated.interestMap[team.name]) {
                        updated.interestMap = {
                            ...updated.interestMap,
                            [team.name]: calculateInterestScore(updated, team, context)
                        };
                    }
                    
                    recruitsById.set(updated.id, updated);
                }
            } else {
                // Already has offer - should we make contact?
                const currentInterest = updated.interestMap[team.name] || 0;
                const shouldContact = currentInterest < 80 && Math.random() < 0.3; // 30% chance to contact
                
                if (shouldContact && pointsSpent + 1 <= budget) {
                    // Contact (1 point) - boost momentum
                    const momentum = updated.teamMomentum || {};
                    momentum[team.name] = (momentum[team.name] || 0) + randomBetween(2, 4);
                    updated.teamMomentum = momentum;
                    pointsSpent += 1;
                    ...
                    allEvents.push({
                        ...
                        week: context.gameInSeason,
                        team: team.name,
                        message: `${team.name} made contact.`,
                        date: context.currentDate,
                        recruitId: updated.id
                    });
    ...
    return {
        updatedRecruits: Array.from(recruitsById.values()),
        events: allEvents
    };
};

/**
 * Full daily recruiting processing including CPU behavior
 */
export const processFullRecruitingDay = (
    recruits: Recruit[],
    allTeams: Team[],
    context: RecruitingContext
): { updatedRecruits: Recruit[], events: TimelineEvent[] } => {
    // 1. Process user/recruit-side actions first
    const playerResult = processRecruitingDay(recruits, context);
    
    // 2. Process CPU team actions on the updated recruits
    const cpuTeams = allTeams.filter(t => !t.isUserTeam);
    const cpuResult = processCPURecruitingDay(cpuTeams, playerResult.updatedRecruits, context);
    
    // 3. Combine events
    const allEvents = [...playerResult.events, ...cpuResult.events];
    
    return {
        updatedRecruits: cpuResult.updatedRecruits,
        events: allEvents
    };
};
...
/**
 * Detect and link package deal partners based on high school
 */
export const detectPackageDeals = (recruits: Recruit[]): Recruit[] => {
    // Group recruits by high school + graduation year
    const groups = new Map<string, Recruit[]>();
    
    for (const recruit of recruits) {
        if (!recruit.highSchoolName) continue;
        
        const key = `${recruit.highSchoolName.toLowerCase()}`;
        
        if (!groups.has(key)) {
            groups.set(key, []);
        }
        groups.get(key)!.push(recruit);
    }
    
    // Update package deal partners for groups of 2+
    const updatedRecruits = recruits.map(r => ({ ...r }));
    const recruitsById = new Map(updatedRecruits.map(r => [r.id, r]));
    ...
    for (const [_, group] of groups) {
        if (group.length === 2) {
            // Perfect pair
            const [r1, r2] = group;
            const updated1 = recruitsById.get(r1.id)!;
            const updated2 = recruitsById.get(r2.id)!;
            
            updated1.packageDealPartner = r2.id;
...
/**
 * Apply package deal momentum bonuses/penalties
 * Called during daily processing
 */
export const applyPackageDealMomentum = (
    recruit: Recruit,
    recruitsById: Map<string, Recruit>,
    context: RecruitingContext
): { updatedRecruit: Recruit, events: TimelineEvent[] } => {
    if (!recruit.packageDealPartner || !recruit.packageDealActive) {
        return { updatedRecruit: recruit, events: [] };
    }
    
    const partner = recruitsById.get(recruit.packageDealPartner);
    if (!partner) {
        return { updatedRecruit: recruit, events: [] };
    }
    ...
    let updated = { ...recruit };
    const events: TimelineEvent[] = [];
    let momentumMod = updated.teamMomentum || {};
    
    // If partner committed, big boost to that school
    if (partner.verbalCommitment) {
        const school = partner.verbalCommitment;
        const currentMomentum = momentumMod[school] || 0;
        
        // +20 momentum bonus (per longprompt.txt)
        if (currentMomentum < 20) {
            momentumMod[school] = Math.min(20, currentMomentum + 20);
            
            events.push({
                type: 'Package',
                week: context.gameInSeason,
                team: school,
                message: `Package deal partner ${partner.name} committed to ${school}. Interest surging!`,
                date: context.currentDate,
                recruitId: recruit.id,
                importance: 75
            });
    ...
    // If partner declined a school, penalty to that school
    if (partner.declinedOffers) {
        for (const declinedSchool of partner.declinedOffers) {
            if (momentumMod[declinedSchool] !== undefined) {
                momentumMod[declinedSchool] = Math.max(-20, (momentumMod[declinedSchool] || 0) - 10);
            }
        }
    }
    
    updated.teamMomentum = momentumMod;
    return { updatedRecruit: updated, events };
};

// ============================================
// PHASE 4 EXTENDED: DECOMMITMENT TRIGGERS
// ============================================

/**
 * Check for decommitment based on various triggers
 */
export const checkForDecommitment = (
    recruit: Recruit,
    context: RecruitingContext & { coachingChanges?: string[] }
): { updatedRecruit: Recruit, events: TimelineEvent[] } => {
    if (!recruit.verbalCommitment) {
        return { updatedRecruit: recruit, events: [] };
    }
    ...
    // Base risk by archetype
    const archetypeRisks: Record<string, number> = {
        'Loyalist': 0.005,
        'Mercenary': 0.03,
        'FameSeeker': 0.02,
        'Wildcard': 0.05,
    ...
    const baseRisk = archetypeRisks[recruit.archetype as string] || 0.01;
    
    let multiplier = 1.0;
    
    // Trigger: Coaching change at committed school
    if (context.coachingChanges?.includes(recruit.verbalCommitment)) {
        multiplier *= 3.0;
    }
    
    // Trigger: Better offer from higher-prestige school
    const committedTeam = context.allTeams.find(t => t.name === recruit.verbalCommitment);
    const committedPrestige = committedTeam?.prestige || 50;
    
    const hasHigherPrestigeOffer = Object.keys(recruit.interestMap).some(teamName => {
        const team = context.allTeams.find(t => t.name === teamName);
        return team && team.prestige > committedPrestige + 15 && recruit.cpuOffers.includes(teamName);
    });
    
    if (hasHigherPrestigeOffer) {
        multiplier *= 1.5;
    }
    
    // Roll for decommitment
    const decommitChance = baseRisk * multiplier;
    
    if (Math.random() < decommitChance) {
        // Decommit!
        const formerSchool = updated.verbalCommitment;
        updated.verbalCommitment = null;
        updated.verbalLevel = 'NONE';
        updated.lockStrength = 0;
        updated.softCommitment = false;
        
        events.push({
            type: 'Decommit',
            week: context.gameInSeason,
            team: formerSchool || '',
            message: `${recruit.name} has reopened his recruitment! (Decommitted from ${formerSchool})`,
            date: context.currentDate,
            recruitId: recruit.id,
            importance: 95
        });
...
// ============================================
// PHASE 6: SEASON RECAP & CPI
// ============================================

export const calculateCoachPerformanceIndex = (team: Team, history: GameState['history'], seasonResults: SeasonRecapData['results']): CoachPerformanceIndex => {
    let score = 50; // Base score
    const components: CoachPerformanceIndex['components'] = [];

    // 1. Regular Season Performance (Wins relative to Prestige)
    // Expected wins roughly correlated with prestige (rough heuristic)
    const expectedWins = Math.round((team.prestige / 100) * 25) + 5; 
    const winDiff = seasonResults.wins - expectedWins;

    if (winDiff > 0) {
        const points = Math.min(25, winDiff * 3);
        score += points;
        components.push({
            label: 'Regular Season Overperformance',
            score: points,
            reason: `${winDiff} wins above expectation (${seasonResults.wins} vs ${expectedWins})`,
            impact: winDiff > 5 ? 'High' : 'Medium'
        });
    } else if (winDiff < 0) {
        const points = Math.max(-25, winDiff * 4); // Penalize losses more
        score += points;
        components.push({
            label: 'Regular Season Underperformance',
            score: points,
            reason: `${Math.abs(winDiff)} wins below expectation (${seasonResults.wins} vs ${expectedWins})`,
            impact: Math.abs(winDiff) > 5 ? 'High' : 'Medium'
        });
    }

    // 2. Tournament Performance
    // Simple logic: early exit is bad for high prestige, deep run is good for everyone
    if (seasonResults.postSeasonResult.includes('Champion')) {
        score += 30;
        components.push({ label: 'National Champions', score: 30, reason: 'Won the title', impact: 'High' });
    } else if (seasonResults.postSeasonResult.includes('Final Four')) {
        score += 20;
        components.push({ label: 'Final Four Run', score: 20, reason: 'Deep tournament run', impact: 'High' });
    } else if (team.prestige > 80 && !seasonResults.postSeasonResult.includes('Sweet 16') && !seasonResults.postSeasonResult.includes('Elite') && !seasonResults.postSeasonResult.includes('Final')) {
         score -= 15;
         components.push({ label: 'Early Tournament Exit', score: -15, reason: 'Failed to meet high standards', impact: 'High' });
    }

    // 3. Recruiting Class - Placeholders for now
    
    // Clamp Score
    score = Math.max(0, Math.min(100, score));

    // Determine Grade & Security
    let grade = 'C';
    let security: CoachPerformanceIndex['security'] = 'Warm';

    if (score >= 90) { grade = 'A+'; security = 'Safe'; }
    else if (score >= 80) { grade = 'A'; security = 'Safe'; }
    else if (score >= 70) { grade = 'B'; security = 'Safe'; }
    else if (score >= 60) { grade = 'C+'; security = 'Safe'; }
    else if (score >= 50) { grade = 'C'; security = 'Warm'; }
    else if (score >= 40) { grade = 'D'; security = 'Hot'; }
    else { grade = 'F'; security = 'Fired'; }
...
export const generateSeasonRecap = (team: Team, history: GameState['history'], financials: Finances, recruitingClass: any): SeasonRecapData => {
    // Helper to calculate or extract results
    const results = {
        wins: team.seasonStats.wins,
        losses: team.seasonStats.losses,
        conferenceRecord: `${team.seasonStats.confWins}-${team.seasonStats.confLosses}`,
        conferenceFinish: team.seasonStats.confFinish || 0,
        postSeasonResult: team.seasonStats.postSeasonResult || 'Missed Tournament',
        finalRank: team.seasonStats.finalRank || 0
    };

    const cpi = calculateCoachPerformanceIndex(team, history, results);
    ...
    return {
        ...
        financials: {
            ...
            primarySource: Object.entries(financials).sort((a,b) => (typeof b[1] === 'number' ? b[1] : 0) - (typeof a[1] === 'number' ? a[1] : 0))[0][0] // Rough calc
        },
        recruiting: {
            classRank: recruitingClass.rank || 0,
            topRecruit: recruitingClass.topInfo || 'None',
            classSize: recruitingClass.size || 0,
            needsMet: true // Placeholder
        },
        prestigeChange: {
            previous: team.prestigeHistory?.[team.prestigeHistory.length - 2]?.value || team.prestige,
            current: team.prestige,
            delta: 0, // Calculate delta
            primaryReason: 'Season Performance'
        },
        rosterChanges: {
            graduating: [], // Placeholder
            transferringOut: [], // Placeholder
            drafted: [], // Placeholder
            returningStarters: 0 // Placeholder
        }
    };
};
