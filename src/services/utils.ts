...
import type { TeamColors, RecruitNilPriority, MotivationKey, ProgramWealth, Team } from '../types';
...
const createDefaultWealth = (team?: Partial<Team>): ProgramWealth => {
    const prestige = team?.prestige ?? 50;
    const fanInterest = team?.fanInterest ?? 50;
    const donationLevel = Math.max(10, Math.round((prestige * 0.6) + (fanInterest * 0.4)));
    const boosterPool = Math.max(0, Math.round(donationLevel / 10));
    const boosterSentiment = clamp(50 + (donationLevel - 50) * 0.6, 0, 100);
    return {
        endowmentScore: prestige,
        donationLevel,
        boosterPool,
        donorMomentum: 0,
        boosterSentiment,
        boosterLiquidity: Math.max(0, boosterPool * 50000),
        alumniNetwork: {
            strength: clamp(Math.round(donationLevel * 0.4), 0, 100),
            lastDonation: 0,
...
export const getProgramWealth = (
    team: Partial<Team> | null | undefined,
    fallback?: () => ProgramWealth
): ProgramWealth => {
    const base = fallback ? fallback() : createDefaultWealth(team || undefined);
    if (!team) return base;
    ...
    if (wealth && typeof wealth === 'object') {
        const merged = {
            ...base,
            ...wealth,
            alumniNetwork: wealth.alumniNetwork
                ? { ...base.alumniNetwork, ...wealth.alumniNetwork }
                : base.alumniNetwork,
        };
        merged.boosterReasons = wealth.boosterReasons ?? base.boosterReasons;
        return merged;
    }

    if (typeof wealth === 'string') {
        const adjustments: Record<string, Partial<ProgramWealth>> = {
            Comfortable: { donationLevel: Math.max(base.donationLevel, 60), boosterPool: Math.max(base.boosterPool, 4) },
            Wealthy: { donationLevel: Math.max(base.donationLevel, 70), boosterPool: Math.max(base.boosterPool, 6) },
            Elite: { donationLevel: Math.max(base.donationLevel, 85), boosterPool: Math.max(base.boosterPool, 8) },
        };
        return { ...base, ...(adjustments[wealth] || {}) };
    }
...
export const schoolNameToSlug = (name: string): string => {
  if (!name) return '';
...
export const describeEndowmentTier = (wealth: any): string => {
    if (typeof wealth === 'string') return wealth;
    // Assuming ProgramWealth object
    return 'Moderate'; // Placeholder logic
};

export const describeDonationLevel = (level: any): string => {
    return 'Standard'; // Placeholder
};

export const describeMomentum = (momentum: any): string => {
