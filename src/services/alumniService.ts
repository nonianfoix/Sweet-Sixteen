import { AlumniProfile, AlumniRegistry, Player, Team, GameState, AlumniArchetype, DonorDilemma, EquityPool, AlumniInfluence } from '../types';
...
export const updateAlumniRegistry = (registry: AlumniRegistry | undefined, newAlumni: AlumniProfile): AlumniRegistry => {
    ...
    // Initial / Default State if undefined
    if (!existing) {
         return { 
    ...
    const isDuplicate = existing?.allAlumni.some(a => a.id === newAlumni.id);
    if (isDuplicate) {
        return existing || { 
            summaryStats: { countsPerProfession: {}, donationMomentum: 0, notableAlumni: [] }, 
            allAlumni: [],
    ...
    // countsPerProfession is already defined above and updated for profession. 
    // Now update for archetype if present.
    if (newAlumni.archetype) {
        countsPerProfession[newAlumni.archetype] = (countsPerProfession[newAlumni.archetype] || 0) + 1;
    }
    ...
    const activeInfluence = { ...INITIAL_INFLUENCE, ...(existing?.activeInfluence || {}) };
    ...
    // donationMomentum is already calculated above.
...
export const generateHistoricalAlumni = (team: Team): AlumniRegistry => {
    ...
    if (realNbaPlayers && realNbaPlayers.length > 0) {
        realNbaPlayers.forEach((realPlayer, index) => {
            ...
            if (!addedIds.has(alumniProfile.id)) {
                alumni.push(alumniProfile);
                addedIds.add(alumniProfile.id);
            }
        });
        ...
        for (let i = 0; i < activeNbaCount; i++) {
            ...
            const mockAlum = generateAlumni(mockPlayer, mockTeamState, gradSeason, 'drafted', earnings);
            if (!addedIds.has(mockAlum.id)) {
                alumni.push(mockAlum);
                addedIds.add(mockAlum.id);
            }
        }
    }
    ...
    for(let i=0; i<baseCount; i++) {
        ...
        const alum = generateAlumni(mockPlayer, mockTeamState, gradSeason);
        if (!addedIds.has(alum.id)) {
            alumni.push(alum);
            addedIds.add(alum.id);
        }
    }
    ...
    if (numTitans > 0) {
        for (let i = 0; i < numTitans; i++) {
             ...
             const mockPlayer = {
                 ...
                 draftProjection: 'Undrafted', 
                 contract: { salary: 0, yearsLeft: 0, type: 'Guaranteed' },
                 injury: null,
                 clutchFactor: 0,
                 consistency: 0,
                 durability: 0,
