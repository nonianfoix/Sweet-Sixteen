...
export const generateSeasonSchedule = (
    ...
    for (const [conf, confTeamIds] of conferences) {
        const n = confTeamIds.length % 2 === 0 ? confTeamIds.length : confTeamIds.length + 1;
        const maxRRRounds = (n - 1) * 2;
        const confTarget = Math.max(0, Math.min(regularSeasonGamesPerTeam, confGamesByConference[conf] ?? 0, maxRRRounds));
...
export const validateSeasonSchedule = (
    ...
    for (const team of teams) {
        ...
        const confTeamIds = conferences.get(team.conference || 'Independent') || [];
        const cn = confTeamIds.length % 2 === 0 ? confTeamIds.length : confTeamIds.length + 1;
        const cMaxRR = (cn - 1) * 2;
        const confTarget = Math.max(0, Math.min(targetGames, confGamesByConference[team.conference || 'Independent'] ?? 0, cMaxRR));
